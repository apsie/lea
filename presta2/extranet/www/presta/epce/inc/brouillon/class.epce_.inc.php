  ...
  if(isset($link_id2[0]) and $link_id2[0]!=NULL)
	{
		...
		while($row = mysql_fetch_array($resultat))
		{
			...
			><a target="blank_" href="../../../index.php?menuaction=addressbook.uicontacts.edit&contact_id='.$id.'">'.$org_name.'</a></td><td width="18%" >'.$fn.'</td><td width="14%">'.$tel_work.'</td><td width="14%">'.$tel_cell.'</td><td width="12%">'.$poste.'</td><td width="10%">'.$qualification.'</td><td width="6%">'.$contrat.'</td><td><a href="../inc/update.php?id_ben='.$choix.'&id='.$id.'&categorie='.$categorie.'&annee='.$annee.'"><img border="0" src="../images/delete.png" /></a></td></tr>';
		}
	}
		echo'</table><br/><br/><br/>';
...
<table width=100%>
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="200px" height="21" class="titre2">Prestation</td><td width="200px" >LC</td><td width="200px">Conseiller</td><td width="200px">Date de debut</td><td width="200px">Date de fin</td><td width="200px">Lieu</td><td width="100px">Statut</td><td width="100px" ></td></tr>';
	...
	function selectionner_rdv($choix)
	{
	  ...
	  echo'<a name="rdv" id="rdv"></a><span class="titre">Ses rendez-vous</span> | <a  href="../pose_rdv.php?choix='.$choix.'">Nouveau rendez-vous</a> | <font color=red>Si les rendez-vous poses n\'apparaissent pas , verifier sur le calendrier que le rendez-vous est linke avec ce beneficiaire (<a target="_blank" href="aide.html">Aide</a>).</font><hr style="border:1px dashed #CCC" />
<table  width="100%">
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="19%" height="21" class="titre2">Intitule du rendez-vous</td><td width="18%" >Date de debut</td><td width="17%">Date de fin</td><td width="17%">Categorie</td><td width="12%">Participants</td><td width="5%">Statut</td></tr>';
		...
		$requete2='SELECT * FROM  egw_cal_dates  where cal_id='.$link_id1.' order by cal_start desc';
	...
	function link_beneficiaire_calendar($id_recup,$opt,$nb,$n_given,$n_family,$tel_work,$tel_cell,$lc,$pole_id,$nom_p,$prenom_p,$tel_home,$code_safir,$tel_contact,$tel_contact2,$date_deb)
	{
...
$resultat= mysql_query($requete) or die(mysql_error());
		...
		$requete2='Update egw_cal set cal_description=" Tel Bureau : '.$tel_work.'\n Tel portable : '.$tel_cell.' \n Tel Privé : '.$tel_home.'\n \n Pôle emploi '.$code_safir.'_'.$pol[0].'\n LC N° '.$lc.' \n ID. '.$pole_id.'
\r Prescripteur : '.$nom_p.' '.$prenom_p.'\n T.'.$tel_contact.' \n F.'.$tel_contact2.' \n\n Lieu : '.$adr_one_street.' '.$adr_one_postalcode.' '.$adr_one_locality.'\n Prestation debut : '.$date_deb.' ",cal_title="'.$ti[0].'_'.$ti[1].'_'.$n_family.' '.$n_given.'" where cal_id='.$opt.'';
		$resultat2= mysql_query($requete2) or die(mysql_error());
   ...
   echo '<table ><tr><td width="100">'.$_j6[$maCle].'</td><td> '.date(' d/m/Y | H:00',$time16[$maCle]).'</td><td >'.date(' d/m/Y | H:00',$time26[$maCle]).'</td><td ><input name="pose[]" value="'.$time16[$maCle].'-'.$time26[$maCle].'" type="checkbox" /></td></tr></table>';
	...
	function chercher_rdv($date_choisi,$selection,$plage1,$plage2,$duree,$conseiller_id,$lieu,$nombre,$jour,$choix,$intervalle)
	{
		...
		*/
		...
		$base= mktime(0,0,0,$chosi[1],$chosi[2],$chosi[0]) + $intervalle;
...
foreach($d1 as $maCle=>$maValeur)
{
 ...
 echo '<table ><tr><td width="50" style="color:#0C0" >'.($this->compteur_date+1).'<input type="hidden" name="date_choisi'.$this->compteur_date.'" value="'.date('d/m/Y',$time1[$maCle]).'"/></td><td width="100">'.$_j[$maCle].'</td><td> '.date(' d/m/Y | H:00',$time1[$maCle]).'</td><td >'.date('d/m/Y | H:00',$time2[$maCle]).'</td><td ><input name="pose'.$this->compteur_date.'" value="'.$time1[$maCle].'-'.$time2[$maCle].'" type="checkbox" /></td></tr></table>';
   
     $this->compteur_date++;
	 if($this->compteur_date<$nombre)
	 {
   $this->chercher_rdv(date('Y/m/d',$base),1,$plage1,$plage2,$duree,$conseiller_id,$lieu,$nombre,$jour,$choix,$intervalle);
	 }
	}
	...
	foreach($d16 as $maCle=>$maValeur)
{
   ...
   echo '<table ><tr><td width="50" style="color:#0C0" >'.($this->compteur_date+1).'<input type="hidden" name="date_choisi'.$this->compteur_date.'" value="'.date('d/m/Y',$time16[$maCle]).'"/></td><td width="100">'.$_j6[$maCle].'</td><td> '.date(' d/m/Y | H:00',$time16[$maCle]).'</td><td >'.date(' d/m/Y | H:00',$time26[$maCle]).'</td><td ><input name="pose'.$this->compteur_date.'" value="'.$time16[$maCle].'-'.$time26[$maCle].'" type="checkbox" /></td></tr></table>';
...
/*	if($z==$nombre)
	...
	function inserer_cal($titre,$lieu,$createur,$conseiller_id)
	{
		...
		elseif($lieu=='Saint-Maur' or $lieu=='Cretail' or $lieu=='Champigny')
		{
...
$requete = "insert into phpgw_p_hours value ('', '$employee', '$project_id', '$activity_id', '$entry_date', '$start_date', '$end_date', '$remark', '$minutes', '$status', '$hours_descr', '$dstatus', '$pro_parent', '$pro_main', '$billable', '$km_distance', '$t_journey', '$cost_id')";
	...
	function inserer_presta($id_ben,$id_ref,$prestataire,$nom,$prenom,$lettre_commande,$presta,$statut,$debut)
	{
		$deb=explode("/",$debut);
	...
	if($module=="NSPP")
	{
	$requete='update  egw_epce_validation set epce="",plan="",bilan="",coherence="",commerciaux="",financier="",juridique="" where id_beneficiaire='.$id.' ';
	}
