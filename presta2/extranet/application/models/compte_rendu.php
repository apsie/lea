...
{		
	...
	public $table_accounts ='apsie_accounts';
 ...
 function get_header()
	 {
		 ...
		 $presta=Prestation::get_prestation($_GET['id_presta']);
		  $contact=Contact::get_contactv2($_GET['id_ben']);
		  $employee=utilisateur::get_employee($presta['id_ref']);
		  $projet=_database::get_projet($_GET['id_ben']);
 ...
 <img style="width:200px;"  src="images/logo_apsie.png" /></div><div style="float:left;width:27%; margin-left:50px;"> <table><tr><td width="110">Bénéficiaire</td><td width="175"> <strong>'.$contact[0].' '.$contact[1].'</strong></td></tr><tr><td>Prestation </td><td><strong>'.$presta['presta'].'</strong></td></tr><tr>
   ...
   <td><strong>'.$employee[0].' '.$employee[1].'</strong></td></tr><tr><td>Compte rendu du </td><td><strong>'.date('d/m/Y',$cal['StartTime']).'</strong></td></tr></table>
</div><div style="float:left;width:45%; margin-left:5px; v"><h3><img src="./images/lumiere.png" />'.utf8_encode(stripslashes($projet['description_projet'])).'</h3></div></div>';
...
function return_action_precedente()
{
	...
	return $this->conn->fetchAll($requete);
	...
	$this->conn->update('egw_compte_rendu_action',$data2,$where2);
	}
	...
	$this->conn->update('egw_compte_rendu_action',$data3,$where3);
	}
	...
	$this->conn->update('egw_compte_rendu_action',$data4,$where4);
	...
	$this->conn->update('egw_compte_rendu_action',$data,$where);
	}
	...
	$this->conn->insert($this->table_compte_rendu,$data);
	}
	 ...
	 function get_nb_compte_rendu($id_projet, $id_presta)
	{
		...
		$result=$this->conn->fetchAll($requete);
		return count($result);
	}
	...
	$this->conn->insert($this->table_theme_aborde,$data);
	}
	/**
	 * @access public
	 */
	...
	$this->conn->insert($this->table_action,$data);
	}	
...
//conseiller
$employee=utilisateur::get_employee($id_employee);
...
//*****Actions planifiées lors du precedent RDV *****
	...
	$result2=$this->conn->fetchAll($requete2);
...
$requete3='SELECT * from egw_compte_rendu_theme_aborde where id_compte_rendu ='.$id_compte_rendu.' order by id asc';
	...
	$result3=$this->conn->fetchAll($requete3);
...
$requete4='SELECT * from egw_compte_rendu_action where id_compte_rendu ='.$id_compte_rendu.' order by id asc';
	...
	$result4=$this->conn->fetchAll($requete4);
...
function imprimer_totalite_milieu($id_employee,$id_ben, $id_projet, $id_presta,$id_compte_rendu, $id_compte_rendu_prec)
	...
	$this->imprimer_compte_rendu_milieu($id_employee,$id_compte_rendu, $id_compte_rendu_prec, $nom_beneficiaire, $prenom_beneficiaire, $nom_complet,'Apsie '.$agence[2], $prestation, $projet,$cal['StartTime']);			
