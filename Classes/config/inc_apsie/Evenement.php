...
<td width="252">'.$result['id_evenement'].'</td></tr><tr><td>Date</td><td>'.date('d/m/Y',$result['date_creation']).'</td></tr><tr>
  <td>Objet</td>
 ...
 <td><select onchange="submit()" name="statut_event"><option>'.$result['statut'].'</option><option>En cours</option><option>Terminé</option></select></td></tr></table><input type="hidden" value="default" name="domain" /><input name="id_evenement" value="'.$id.'" type="hidden" /></form><br/>
  ...
  echo'<strong>'.date('d/m/Y',$result[$i]['date']).'</strong> - <img src="images/user_business.png" /> '.$val_messager[0].' '.$val_messager[1].'<br/>'.$result[$i]['message'].'<br/><br/>';
	}
