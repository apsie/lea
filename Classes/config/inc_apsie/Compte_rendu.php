...
class Compte_rendu
{
    ...
    function get_header()
    {
		...
		<img style="width:200px;"  src="images/logo_apsie.png" /></div><div style="float:left;width:27%; margin-left:50px;"> <table><tr><td width="110">B�n�ficiaire</td><td width="175"> <strong>'.$contact[0].' '.$contact[1].'</strong></td></tr><tr><td>Prestation </td><td><strong>'.$presta['intitule'].'</strong></td></tr><tr>
    ...
    function get_rdv_precedent()
    {
        if (isset($_GET['id_projet']) and isset($_GET['id_presta']) and $this->get_nb_compte_rendu($_GET['id_projet'], $_GET['id_presta'])!=0) {
            ...
            echo'Actions planifi�es lors du RDV pr�c�dent : <br/><br/>
<table style="border:1px solid #CCC;"><tr style="font-size:12px; font-weight:bolder;color:#FFF " align="center" bgcolor="#FDCE6F">
<td width="180px">Th�mes</td><td width="180px"><strong>Demarches/actions pr&eacute;vues&nbsp;:</strong></td> <td width="180px">A faire par</td>
<td width="180px">Objectif :</td>
<td width="180px">R�sultat :</td><td width="180px">Observation :</td></tr>';
...
//Ouvre le fichier mod�le
        $filename='/data/html/egw_apsie_143/lea/Compte_rendu1_0/doc/premier_compte_rendu.rtf';
        $fp=fopen($filename, 'r+');
...
//*****Actions planifi�es lors du precedent RDV *****
...
//*****Actions � r�aliser pour le prochain RV :*****
...
//*****Documents �labor�s au cours de la s�ance  :*****
...
//*****supports communiqu�s au cours de la s�ance  :*****
...
//Envoie le document produit au navigateur
    ...
    function imprimer_totalite_premier($id_employee, $id_ben, $id_projet, $id_presta, $id_compte_rendu)
    {
        ...
        $requete='SELECT intitule FROM  egw_prestation  where id_presta='.$id_presta.'';
        $result=$GLOBALS['db']->fetchRow($requete);
...
//Ouvre le fichier mod�le
        $filename='/data/html/egw_apsie_143/lea/Compte_rendu1_0/doc/compte_rendu_milieu.rtf';
        $fp=fopen($filename, 'r+');
//Stocke le mod�le dans une variable
        $output=fread($fp, filesize($filename));
