...
require_once(EGW_INCLUDE_ROOT. '/spireapi/inc/class.facture_categories_bo.inc.php');	
...
class facture_categories_ui extends facture_categories_bo
{
	...
	*/
		...
		$this->prefs =& $GLOBALS['egw_info']['user']['preferences']['spireapi'];
	...
	*/
		...
		if (!is_array($content['nm']))
		{
			...
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spireapi.facture_categories_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
		...
		if(!isset($_GET['view'])){
			$content['nm']['header_right'] = 'spireapi.facture_categories.index.right';
		}
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Invoice Categories Management');
		
		$tpl = new etemplate('spireapi.facture_categories.index');
		$tpl->exec('spireapi.facture_categories_ui.index', $content,$sel_options,$no_button, $content);
	}
	...
	*/
		...
		if(isset($id)){
			...
			if(empty($id)){
				...
				$content['cat_active'] = true;
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Add invoice category');	
			}else{
				$content+=$this->get_info($id);
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Edit invoice category');	
			}
		}
		...
		$tpl = new etemplate('spireapi.facture_categories.edit');
		$tpl->exec('spireapi.facture_categories_ui.edit', $content,$sel_options,$readonlys,$content,2);
	}	
}
