...
class trans_bo extends trans_so{
	 ...
	 */
		...
		$so_applications = new so_sql('phpgwapi', 'egw_applications');
		$join = 'WHERE app_enabled != 3 and app_enabled > 0';
		$applications = $so_applications->search($search,'app_id,app_name','app_name ASC','',$wildcard,false,$op,$start,$query['col_filter'],$join);
