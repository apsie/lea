 ...
 * @version $Id: class.calendar_so.inc.php 54153 2015-11-03 13:43:30Z ralfbecker $
 */
...
// spirea - ajout statuts
define('ABSTEXCUSE',98);
define('ABSTNONEXCUSE',99);
// spirea - ajour non excusée
...
class calendar_so
{
	...
	const STATUS_SORT = "CASE cal_status WHEN 'U' THEN 1 WHEN 'T' THEN 2 WHEN 'A' THEN 3 WHEN 'R' THEN 4 WHEN 'O' THEN 98 WHEN 'N' THEN 99 ELSE 0 END ASC";
	...
	protected function status_filter($filter, $enum_recuring=true, array $where=array())
	{
		...
		switch($filter)
		{
			...
			// spirea TCH - ajout statut	
			case 'absent_excuse':
				$where[] = "$this->user_table.cal_status='O'";
				break;
			case 'absent_non_excuse':
				$where[] = "$this->user_table.cal_status='N'";
				break;
			// fin 
	 ...
	 */
	// SPIREA - YLF (ajout $site)
	function &search($start,$end,$users,$cat_id=0,$filter='all',$offset=False,$num_rows=0,array $params=array(),$remove_rejected_by_user=null,$site=0)
	{
		//error_log(__METHOD__.'('.($start ? date('Y-m-d H:i',$start) : '').','.($end ? date('Y-m-d H:i',$end) : '').','.array2string($users).','.array2string($cat_id).",'$filter',".array2string($offset).",$num_rows,".array2string($params).') '.function_backtrace());
...
*/
	...
	function set_status($cal_id,$user_type,$user_id,$status,$recur_date=0,$role=null,$attendee=null)
	{
		static $status_code_short = array(
			...
			ACCEPTED	=> 'A',
			DELEGATED	=> 'D',
			// Spirea - tch - juin 2017 - ajout statuts
			ABSTEXCUSE		=> 'O',
			ABSTNONEXCUSE	=> 'N',
			// fin ajout statut
