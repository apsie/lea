 ...
 * @version $Id: class.calendar_bo.inc.php 53911 2015-10-12 11:55:39Z ralfbecker $
 */
...
class calendar_bo
{
	...
	function __construct()
	{
		...
		// spirea - tch - ajout de apsieben dans la liste des applications cherchées via les hooks...
		// if (!is_array($this->resources = $GLOBALS['egw']->session->appsession('resources','calendar')))
		if (!is_array($this->resources = $GLOBALS['egw']->session->appsession('resources','calendar','apsieben')))
		{
	...
	function &search($params,$sql_filter=null)
	{
		...
		//Spirea-YLF
		$site = isset($params['site']) ? $params['site'] : 0;
		...
		$events =& $this->so->search(isset($start) ? $this->date2ts($start,true) : null,isset($end) ? $this->date2ts($end,true) : null,
			$users,$cat_id,$filter,$offset,(int)$params['num_rows'],$params,$remove_rejected_by_user,$site);
		...
		if (!isset($id2lid[$id]))
		{
			if (!is_numeric($id))
			{
				...
				if ($id[0] == 'b')
				{
					...
					$id_ben = substr($id,1);
					$id2lid[$id] = $apsieben_bo->link_title($id_ben);	
				}
				// fin spirea
			}
			else
