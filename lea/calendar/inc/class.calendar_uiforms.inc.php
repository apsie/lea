 ...
 * @version $Id: class.calendar_uiforms.inc.php 54057 2015-10-26 21:43:34Z ralfbecker $
 */
...
class calendar_uiforms extends calendar_ui
{
	...
	function edit($event=null,$preserv=null,$msg='',$link_to_id='',$msg_type=null)
	{
		...
		$sel_options = array(
			...
			// SPIREA - YLF
			'site' => /*array('' => lang('Select one')) +*/ $site_bo->get_all_sites(true),
		);
		...
		foreach($event['participant_types'] as $type => $participants)
		{
			...
			foreach($participants as $id => $status)
			{
				...
				elseif ($name == 'apsieben')
				{
					$current_row = $row - 1;
					$apsieben_bo = CreateObject('apsieben.apsieben_bo');
					$id_ben = substr($content['participants'][$current_row]['uid'],1);
					$readonlys['participants'][$current_row]['status'] = false;
					$content['participants'][$current_row]['title'] = $apsieben_bo->link_title($id_ben); // Pour permettre la modification 
					// des statuts - pour debug : 	_debug_array($readonlys); (en dessous du dernier else)
