 ...
 * @version $Id: class.calendar_uiviews.inc.php 53881 2015-10-09 07:25:55Z hnategh $
 */
...
class calendar_uiviews extends calendar_ui
{
	...
	{
		// Spirea-YLF - traitement du formulaire

		if(!empty($_POST['submit'])){
			$tpost = $_POST;
			unset($tpost['submit']);
			$part=array();
			foreach($tpost as $id_evt_part => $cal_status){
				// echo $part[0];
				$partt = explode('-',$id_evt_part);
				$cal_id=$partt[0];
				$user_id=$partt[1];
				// $part['cal_status']=$part[0];
				$GLOBALS['egw']->db->update('egw_cal_user',array(
		...
		// _debug_array($tpost);exit;
			
		}
		unset($tpost);
		// fin spirea - traitement maj
		...
		if (!$home)
		{
			...
			// Spirea - TCH - Statistiques sur les RV / juin 2017
			// _debug_array($search_params);
			$this->spirea_stats($events);
			// fin stats
	...
	function week($days=0,$home=false)
	{
		...
		if (!$home)
		{
			...
			// Spirea - TCH - Statistiques sur les RV / juin 2017
			// _debug_array($search_params);
			$search_params['daywise']='';
			$search_params['users']=$this->search_params['users'];
			$events =& $this->bo->search($search_params);
			// _debug_array($events);
			$this->spirea_stats($events);
			// fin stats
	...
	function eventWidget($event,$width,$indent,$owner,$return_array=false,$block='event_widget',$z_index=null)
	{
		...
		//Spirea-YLF - Permet d'ajouter le site dans la mini info de l'évènement
		$site = CreateObject('spireapi.site_bo');
		$sites  = $site->get_info($event['site']);
		$color = $sites['site_color'];
		$site = $sites['site_label'];
		...
		$tpl->set_var(array(
			...
			//Spirea-BBO - 19/01/2010 - Permet d'ajouter le site dans la mini info de l'évènement
			'site' => $this->add_nonempty($site,lang('Site')),
...
*/
	...
	*/
	...
	function spirea_stats($events=array()){
		...
		$verbose_status = array(
			...
			$total_part = 0;
			$total_temps_rv = 0;
			$total_tps_part_u = $total_tps_part_a = $total_tps_part_r = $total_tps_part_0 = $total_tps_part_n = 0;
			$total_tps_part_u = 0;
			$part_u = $part_a = $part_r = $part_0 = $part_n = 0;
			$rv_prod = 0;
			...
			foreach($events as $evt){
				$rvprodavecben = false;
				$part_rv = false;
				$duree_rv = $evt['end'] - $evt['start'];
				
				
				foreach($evt['participants'] as $id_part => $part){
						// echo $id_part;
						// if(!is_int($id_part)){
							
						if(is_int($id_part)){
							$conseiller = $id_part;
							$tableau_participant[$evt['id']]['participants'][]=$id_part;
						}
							
						if($id_part[0] == 'b'){
							$rvprodavecben = true;
							++$total_part;
							$tableau_participant[$conseiller][$evt['id']]['ben']=$id_part;
							$tableau_participant[$conseiller][$evt['id']]['status']=$part;
							$tab_conseiller[$conseiller][$part]['nb_rvprod']=$tab_conseiller[$conseiller][$part]['nb_rvprod']+1;
							$tab_conseiller[$conseiller][$part]['tps_part']=$tab_conseiller[$conseiller][$part]['tps_part']+$duree_rv;
							$tab_conseiller[$conseiller][$part]['H'] = floor($tab_conseiller[$conseiller][$part]['tps_part'] / 3600);
							$tab_conseiller[$conseiller][$part]['i'] = ($tab_conseiller[$conseiller][$part]['tps_part'] / 60) % 60;
							$tab_conseiller[$conseiller][$part]['s'] =$tab_conseiller[$conseiller][$part]['tps_part'] % 60;
	
							$tableau_participant[$evt['id']]['ben']=$id_part;
							$tableau_participant[$evt['id']]['status']=$part;
							$tableau_participant[$evt['id']]['start']=$evt['start'];
							switch($part)
							{
								case 'A':
									++$part_a;
									$part_rv = true;
									$total_tps_part_a += $duree_rv;
									$total_tps_part += $duree_rv;
									break;
								case 'U':
									++$part_u;
									$total_tps_part_u += $duree_rv;
									break;
								case 'R':
									++$part_r;
									$total_tps_part_r += $duree_rv;
									break;
								case 'N':
									++$part_n;
									$total_tps_part_n += $duree_rv;
									break;
								case 'O':
									++$part_o;
									$total_tps_part_o += $duree_rv;
								break;
							}
						}
				...
				if(is_numeric(substr($evt['title'],0,6))){
					++$rvprod;
					if($rvprodavecben == false){
						++$rvprodsansben;
					}
				}
				
				if($part_rv == true){
						$total_temps_rv += $duree_rv;
				}
			...
			// _debug_array($events);
			// echo $total_tps_part_a;
			
			echo "<strong>Statistiques de la vue active</Strong>";
			echo "<br>Nombre de RV : ".count($events);
			echo "<br>Nombre de RV de prod: ".$rvprod;
			echo "<br>Nombre de RV de prod sans bénéficiaire: ".$rvprodsansben;
						
			echo "<br>Participations/absences : ".$total_part;
			// _debug_array($tableau_participant);
			// $debtableaustats = "<table border=\"1px solid black\"><tr>";
			$debtableaustats = "<table class=\"table\"><tr style=\"border-bottom:1pt solid black; !important\">";
			$debtableaustats .= "<td><strong></td>";
			$debtableaustats .= "<td><strong>Présent</td>";
			$debtableaustats .= "<td><strong>Rejeté</td>";
			$debtableaustats .= "<td><strong>Excusé</td>";
			$debtableaustats .= "<td><strong>Non Excusé</td>";
			$debtableaustats .= "<td><strong>Pas de réponse</td>";
			$debtableaustats .= "</tr>";
			
			$tableaustats .= "<tr>";
			$tableaustats .= "<td><strong>Participations/Absence</td>";
			$tableaustats .= "<td>".$part_a."</td>";
			$tableaustats .= "<td>".$part_r."</td>";
			$tableaustats .= "<td>".$part_o."</td>";
			$tableaustats .= "<td>".$part_n."</td>";
			$tableaustats .= "<td>".$part_u."</td>";
			$tableaustats .= "</tr>";
			$tableaustats .= "<tr>";
			$tableaustats .= "<td><strong>Heures de RV</td>";
			
			$A = $R = $O = $N =$U = '';
			if($total_tps_part_a > 0){
				$H = floor($total_tps_part_a / 3600);
				$i = ($total_tps_part_a / 60) % 60;
				$s = $total_tps_part_a % 60;
				$A = sprintf("%02d:%02d:%02d", $H, $i, $s);
			}
			
			
			if($total_tps_part_r > 0){
				$H = floor($total_tps_part_r / 3600);
				$i = ($total_tps_part_r / 60) % 60;
				$s = $total_tps_part_r % 60;
				$R = sprintf("%02d:%02d:%02d", $H, $i, $s);
			}
			
			
			if($total_tps_part_o > 0){
				$H = floor($total_tps_part_o / 3600);
				$i = ($total_tps_part_o / 60) % 60;
				$s = $total_tps_part_o % 60;
				$O = sprintf("%02d:%02d:%02d", $H, $i, $s);
			}
			
			

			if($total_tps_part_n > 0){
				$H = floor($total_tps_part_n / 3600);
				$i = ($total_tps_part_n / 60) % 60;
				$s = $total_tps_part_n % 60;
				$N = sprintf("%02d:%02d:%02d", $H, $i, $s);
			}
			
			
			
			if($total_tps_part_u > 0){
				$H = floor($total_tps_part_u / 3600);
				$i = ($total_tps_part_u / 60) % 60;
				$s = $total_tps_part_u % 60;
				$U = sprintf("%02d:%02d:%02d", $H, $i, $s);
			}
			
			$tableaustats .= "<td>".$A."</td>";
			$tableaustats .= "<td>".$R."</td>";
			$tableaustats .= "<td>".$O."</td>";
			$tableaustats .= "<td>".$N."</td>";
			$tableaustats .= "<td>".$U."</td>";
			$tableaustats .= "</tr>";
			
			echo $debtableaustats.$tableaustats.'<tr><td colspan="6">&nbsp;</td></tr>';
			
			// echo '<tr><td colspan="6"></td>ici</tr>';
			unset($tableaustats);
			
			// _debug_array($tab_conseiller);
			$spireapi_employee_bo = CreateObject('spireapi.employee_bo');
			foreach($tab_conseiller as $id_conseiller => $part_conseille){
					
					
					$employee = $spireapi_employee_bo->get_info($id_conseiller);
					$account_n = $GLOBALS['egw']->accounts->read($id_conseiller);
					// _debug_array($employee);
					$debtableaustats = "<tr style=\"border-bottom:1pt solid black;\">";
					$debtableaustats .= "<td><strong>".$account_n['account_fullname']."</td>";
					$debtableaustats .= "<td><strong>Présent</td>";
					$debtableaustats .= "<td><strong>Rejeté</td>";
					$debtableaustats .= "<td><strong>Excusé</td>";
					$debtableaustats .= "<td><strong>Non Excusé</td>";
					$debtableaustats .= "<td><strong>Pas de réponse</td>";
					$debtableaustats .= "<td><strong>Objectif (semaine)</td>";
					$debtableaustats .= "</b></tr>";
			
				
				
					$tableaustats = "<tr>";
					$tableaustats .= "<td><strong>Participations/Absence</td>";
					$tableaustats .= "<td>".$part_conseille['A']['H']."</td>";
					$tableaustats .= "<td>".$part_conseille['R']['H']."</td>";
					$tableaustats .= "<td>".$part_conseille['O']['H']."</td>";
					$tableaustats .= "<td>".$part_conseille['N']['H']."</td>";
					$tableaustats .= "<td>".$part_conseille['U']['H']."</td>";
					$tableaustats .= "<td>".($employee['employee_week_time']*$employee['employee_hour_rate']/100)."</td>";
					$tableaustats .= "</tr>";
					$tableaustats .= "<tr>";
					...
					$A = $R = $O = $N =$U = '';
					// echo "ici".$A;
					if($tab_conseiller[$id_conseiller]['A']['tps_part']>0){
						$A = sprintf("%02d:%02d:%02d", $part_conseille['A']['H'], $part_conseille['A']['i'], $part_conseille['A']['s']);
						
					}
					if($tab_conseiller[$id_conseiller]['R']['tps_part']>0){
						$R = sprintf("%02d:%02d:%02d", $part_conseille['R']['H'], $part_conseille['R']['i'], $part_conseille['R']['s']);
					}
					if($tab_conseiller[$id_conseiller]['O']['tps_part']>0){
						$O = sprintf("%02d:%02d:%02d", $part_conseille['O']['H'], $part_conseille['O']['i'], $part_conseille['O']['s']);
					}
					if($tab_conseiller[$id_conseiller]['N']['tps_part']>0){
						$N = sprintf("%02d:%02d:%02d", $part_conseille['N']['H'], $part_conseille['N']['i'], $part_conseille['N']['s']);
					}
					if($tab_conseiller[$id_conseiller]['U']['tps_part']>0){
						$U = sprintf("%02d:%02d:%02d", $part_conseille['U']['H'], $part_conseille['U']['i'], $part_conseille['U']['s']);
					}
					$tableaustats .= "<td>".$A."</td>";
					$tableaustats .= "<td>".$R."</td>";
					$tableaustats .= "<td>".$O."</td>";
				...
				echo $debtableaustats.$tableaustats;
			}
			echo '</table><br>';
			
			if($part_u>0){
				$tableau = "<form method='post'><table border='1' style='border: 1px solid black;'><tr><td><b>Evt</td><td><strong>Bénéficiaire</td><td><strong>Statut</strong></td>";
				$tableau .= "<td><strong>Participants</td>";
				$tableau .= "<td><strong>Présent</td>";
				$tableau .= "<td><strong>Excusé</td>";
				$tableau .= "<td><strong>Non Excusé</td>";
				$tableau .= "<td><strong>Pas de réponse</td>";
				$tableau .= "</b></tr>";
				
				$apsieben_bo = CreateObject('apsieben.apsieben_bo');
				// _debug_array($tableau_participant);
				foreach($tableau_participant as $id_evt => $part){
					// Spirea - tch - affichage du nom du bénéficiaire sur les fonctions uiviews du calendrier
					
					if ($part['status'] == 'U' && $part['ben'][0] == 'b' && $part['start']<time())
					{
						$id_ben = substr($part['ben'],1);
						$nom_ben = $apsieben_bo->link_title($id_ben);	
						// $tableau .= "<tr><td>".$id_evt."</td><td>".$part['ben']."</td><td>".lang($verbose_status[$part['status']])."</td>";
						$tableau .= "<tr><td>".date("d-m H:i",$part['start'])."</td><td>".$nom_ben."</td><td>".lang($verbose_status[$part['status']])."</td>";

						// Affichage des utilisateurs du rendez-vous
						$tmp_participants = array();
						foreach($part['participants'] as $particpant_id){
							$account_n = $GLOBALS['egw']->accounts->read($particpant_id);
							$tmp_participants[] = $account_n['account_fullname'];
						}
						$tableau .=  '<td>'.implode("<br>", $tmp_participants).'</td>';						

						$tableau .=  '<td><input name='.$id_evt.'-'.$id_ben.' value="A" type="radio"></td>';
						$tableau .=  '<td><input name='.$id_evt.'-'.$id_ben.' value="O" type="radio"></td>';
						$tableau .=  '<td><input name='.$id_evt.'-'.$id_ben.' value="N" type="radio"></td>';
						$tableau .=  '<td><input name='.$id_evt.'-'.$id_ben.' value="U" type="radio"></td>';
					}
					
					
					$tableau .= "</tr>";
				}
				echo $tableau.'</table><br>'.html::submit_button('submit','Submit').'</form>';
			}
			
			// $lang_status = lang($this->calendar_bo->verbose_status);
			// _debug_array($this->calendar_bo->verbose_status);
			// _debug_array($lang_status);
			
			// Fin Spirea Statistiques
