...
class generate_pdf extends fpdf{
	...
	*/
		...
		$this->Ln(5);
		$this->SetFont('Arial','',8);
		$this->Cell(0,5,utf8_decode($this->societeEmetteur['client_footer_one']),0,0,'C');
		$this->Ln(5);
		$this->Cell(0,5,utf8_decode($this->societeEmetteur['client_footer_two']),0,2,'C');
		...
		if(!empty($this->facture_ui->obj_config['invoice_payment'])){
			$this->MultiCell(0,5,utf8_decode(lang('PAYMENT').' : '.str_replace('<br />', "\n", $this->facture_ui->obj_config['invoice_payment'])),0,$formText);
		...
		if(!empty($this->facture_ui->obj_config['invoice_payment'])){
			$this->MultiCell(0,5,utf8_decode(lang('PROPERTY').' : '.str_replace('<br />', "\n", $this->facture_ui->obj_config['invoice_property'])),0,$formText);
		}
	}
