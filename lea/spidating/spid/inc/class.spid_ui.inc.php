...
class spid_ui extends spid_bo 
{
	...
	*/
		...
		$content['nm'] = $GLOBALS['egw']->session->appsession($save_session,'spid');
		...
		{			
			...
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spid.spid_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
	 ...
	 */
		...
		if(!$this->groupeGestionnaire()){
			if(!empty($this->tab_technicienCategorie)){
				if(empty($query['col_filter']['cat_id'])){
					$recherche[]= 'spid_tickets.cat_id IN ('.implode(',', $this->tab_technicienCategorie).')';
				}
		...
		foreach($rows as $id=>$value){
			...
			if($value['facture_id']>0)
			{
				$rows[$id]['link_facture']=',spid.facture_ui.view&id=$row_cont[facture_str]&client=$row_cont[client_id],,,,900x720,$row_cont[facture_str]';
			}
	...
	*/
		...
		// }
		//Vérification que le ticket en cours d'ouverture et appartient bien au groupe du user connecté quand ce dernier ne fait partie que d'un groupe
		...
		if(!empty($content['ticket_id'])){
			...
			if(!in_array($content['cat_id'],$GestionnaireCategorie) && !in_array($GLOBALS['egw_info']['user']['person_id'],array_keys($this->get_demandeur($content['client_id'])))) {
	...
	*/
		...
		$ticketForfait['ticket_spend_time']=0;
		...
		echo $GLOBALS['egw_info']['flags']['java_script'];
		//exit;
		return $msg;
	}
