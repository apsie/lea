...
class generate_pdf_green extends fpdf{
	...
	*/
		if(file_exists(EGW_INCLUDE_ROOT.'/spifina/templates/default/images/green_footer_middle_'.$this->societeEmetteur['client_id'].'.jpg')){
			$this->Image(EGW_INCLUDE_ROOT.'/spifina/templates/default/images/green_footer_middle_'.$this->societeEmetteur['client_id'].'.jpg',95,268,20);
		}else{
			$this->Image(EGW_INCLUDE_ROOT.'/spifina/templates/default/images/green_footer_middle.jpg',95,268,20);	
		}
		
		$this->Image(EGW_INCLUDE_ROOT.'/spifina/templates/default/images/green_footer_right.jpg',170,280,25);
...
// _debug_array($LignePerso);
		...
		$this->SetX(55);
		$this->Cell(100,12,utf8_decode(lang('Invoice n°').' '.$this->facture[0]['facture_number']),1,1,'C',0);
		$this->Ln(5);
		...
		$this->SetFont('','');
		$this->SetX(35);
		$this->MultiCell(140,4,utf8_decode($this->facture[0]['invoice_message']),0,'C',0);
		$this->Ln(2);
		...
		foreach($FacturationTableau as $data){
			...
			$this->SetX(35);
			$this->Cell(140,4,utf8_decode(lang('Intervention').' : '.$ticket['ticket_title']),0,1,'C',0);
			$this->Ln(1);
		...
		foreach($LignePerso as $data){
			$this->SetX(35);
			$this->Cell(140,4,utf8_decode($data['extra_ref'].' '.$data['extra_ns']),0,1,'C',0);

			$this->SetX(35);
			$this->Cell(140,4,utf8_decode($data['extra_label']),0,1,'C',0);
			$this->Ln(1);
		...
		foreach($vat as $vat_rate => $vat_amount){
			if($vat_rate != 0 || $vat_rate == 'ht'){
				...
				if($vat_rate == 'ht'){
					...
					$amount_text = lang('AMOUNT').' '.$vat_rate.'%';
				}

				$size_text = $this->GetStringWidth(utf8_decode($amount_text));
				$this->Cell($size_text,4,utf8_decode($amount_text),0,0,'L',0);
				
				$size_amount = 17;

				$this->SetX(35 + $size_text);
				$fill_text = '.';
				while($this->GetStringWidth(utf8_decode($fill_text)) + $size_text + $size_amount < 140){
					$fill_text .= '.';
				}
				// _debug_array($fill_text);
				$this->Cell($size_text,4,utf8_decode($fill_text),0,0,'L',0);

				$this->SetX(35 + $size_text + $this->GetStringWidth(utf8_decode($fill_text)));
				...
				$this->Cell($size_amount,4,utf8_decode(number_format($amount,2,',',' ').' ').chr(128),0,1,'R',0);

				$facture_total += $amount;
			}
		}

		$conv = new ChiffreEnLettre();
   		$tabmontant = explode('.',number_format($facture_total,2,'.',''));
   		$montantlettre = $conv->Conversion($tabmontant[0]).'EUROS';
   		if($tabmontant[1] != 0){
   			$conv = new ChiffreEnLettre();
   			$montantlettre .= ' ET '.$conv->Conversion($tabmontant[1]).'CTS';
   		}

		$this->SetX(35);
		$this->Cell(140,4,utf8_decode('---------------------'),0,1,'R',0);
		...
		$this->SetX(35 + $size_text + $this->GetStringWidth(utf8_decode($fill_text)));
		$this->Cell($size_amount,4,utf8_decode(number_format($facture_total,2,',',' ').' ').chr(128),0,1,'R',0);

		$this->Ln();
		$this->SetX(35);
		$text = lang('Arrêtée la présente facture à la somme').' <b>'.$montantlettre.'</b> '.lang('Toutes Taxes Comprises');
		$this->WriteHTML(utf8_decode($text));

		$this->Ln();
		$this->SetX(35);
		// _debug_array($this->societeClient);
		$bank_suffix = $facture[0]['invoice_bank_account'];
		if(!empty($this->societeEmetteur['client_banque'.$bank_suffix]))
			$temp_account[] = $this->societeEmetteur['client_banque'.$bank_suffix];
		if(!empty($this->societeEmetteur['client_agency'.$bank_suffix]))
			$temp_account[] = $this->societeEmetteur['client_agency'.$bank_suffix];
		if(!empty($this->societeEmetteur['client_bic'.$bank_suffix]))
			$temp_account[] = lang('BIC').': '.$this->societeEmetteur['client_bic'.$bank_suffix];
		if(!empty($this->societeEmetteur['client_iban'.$bank_suffix]))
			$temp_account[] = lang('IBAN').': '.$this->societeEmetteur['client_iban'.$bank_suffix];

		$this->MultiCell(140, 4, utf8_decode('En votre aimable règlement, '.$modePaiementClient.' au compte suivant : '.implode(' - ',$temp_account)));

		$this->Ln();
		$this->SetX(35);
		$this->SetFont('','B');
		$this->Cell(140, 4, utf8_decode(lang('N° T.V.A intracommunautaire').' : '.$this->societeEmetteur['client_tva_number']),0,2);
		$this->Cell(140, 4, utf8_decode(lang('T.V.A acquittée sur les encaissements')),0,2);
		$this->Cell(140, 4, utf8_decode(lang('T.V.A récupérable après paiement')),0,2);

		$this->Ln(15);
		$this->SetFont('','');
		if(!empty($this->facture_ui->obj_config['invoice_payment']) && $this->facture_ui->obj_config['invoice_payment'] != '<br />'){
			$this->SetX(35);
			$this->MultiCell(140,4,utf8_decode(str_replace('<br />', "\n", $this->facture_ui->obj_config['invoice_payment'])),0,$formText);
