...
*/
require_once(EGW_INCLUDE_ROOT. '/spifina/inc/class.line_bo.inc.php');
require_once(EGW_INCLUDE_ROOT. '/spireapi/inc/class.vat_bo.inc.php');
// require_once(EGW_INCLUDE_ROOT. '/spireapi/inc/class.factture_bo.inc.php');
...
class line_ui extends line_bo{
	 ...
	 */
		...
		{
			$default_cols='detail_id,facture_number,facture_id,extra_cat_id,extra_label,extra_ht,extra_rank,extra_ref,extra_ns,extra_ht_unit,vat_id,vat_rate,quantity';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spifina.line_bo.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
		...
		$sel_options = array(
			'extra_cat_id' => $fact_cat_bo->get_cat_facture(),
			'vat_id' => array(''=>'') + vat_bo::get_vat(),

			'filter' => array('' => lang('All')) + $fact_cat_bo->get_cat_facture(),
			'filter2' => $client_id,
			'client' => $client_id,
		...
		$content['nm']['header_left'] = 'spifina.line.index.left';
		
		$tpl = new etemplate('spifina.line.index');
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Custom lines view');
		$tpl->read('spifina.line.index');
		$tpl->exec('spifina.line_ui.index', $content, $sel_options, $readonlys, array('nm' => $content['nm']));
	}
}
?>