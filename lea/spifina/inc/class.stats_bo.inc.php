...
*/		
require_once(EGW_INCLUDE_ROOT. '/spifina/inc/class.stats_so.inc.php');	
...
class stats_bo extends stats_so{
	...
	* @return array
	*/
		$info = $this->so_client->search('','client_id,client_company');
		if(!is_array($info)){
			$info=array();
		}
		$societe=array();
	...
	*/
	...
	function get_cat_budget_ticket(){
	/**
	 * Retourne les catégories des budgets et des tickets (si spid est installé)
	 *
	 * @return array
	 */
		$return = array();
		$cat_budget = $this->so_cat_invoice->search('',false);
		foreach($cat_budget as $cat){
			$return['b'.$cat['cat_id']] = $cat['cat_name'];
		}

		if($GLOBALS['egw_info']['apps']['spid']){
			$GLOBALS['egw']->categories->categories('','spid');
			$cats = $GLOBALS['egw']->categories->return_array('all',0,false,'','','',true);
			foreach($cats as $cat){
				$return['t'.$cat['id']] = $cat['name'];
			}
		}
