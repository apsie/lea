...
//RDV
		...
		{
		$req_cal=$req_cal.' or Id='.$cal_id[$i];
		}
		$requete='SELECT Id FROM  apsie_jqcalendar where (Id=0 '.$req_cal.' ) and cal_status!="R"';
		...
		{
		$req_cal2=$req_cal2.' or Id='.$cal_id2[$i];
		}
		$requete2='SELECT * FROM  apsie_jqcalendar  where Id=0 '.$req_cal2.' order by StartTime asc';
		...
		while($row = mysql_fetch_array($resultat2))
		{
			...
			$cal_end_ben[]=$row['startTime'];
			$cal_start_ben[]=$row['EndTime'];
	...
	$requete='SELECT Id FROM  apsie_jqcalendar where (Id=0 '.$req_cal.' ) and cal_status="R"';
		...
		for ($i=0;$i<count($cal_id);$i++)
		{
		$req_cal2=$req_cal2.' or Id='.$cal_id[$i];
		}
		$requete2='SELECT * FROM  apsie_jqcalendar  where Id=0 '.$req_cal2.' order by StartTime asc';
		...
		while($row = mysql_fetch_array($resultat2))
		{
			...
			$cal_end_ben[]=$row['EndTime'];
			$cal_start_ben[]=$row['StartTime'];
...
/*$requete='SELECT * FROM  egw_links  where link_app1="calendar" and link_app2="addressbook" and link_id2='.$id_beneficiaire.' ';
		...
		$requete='SELECT cal_id FROM  apsie_jqcalendar_user where (cal_id=0 '.$req_a.')  and (cal_status="A")';
...
$requete='SELECT * FROM  apsie_jqcalendar J
LEFT JOIN apsie_lieu L ON L.id_lieu=J.id_lieu  where Subject like "%'.$this->nom_global.' '.$this->prenom_global.'" order by Id desc';
		$resultat = mysql_query($requete) or die(mysql_error());
		while($row = mysql_fetch_array($resultat))
		{
				$this->p_org_name=$row['nom_lieu'];
				$this->p_adr_one_street=$row['adresse_lieu'];
				$this->p_adr_one_locality=$row['ville_lieu'];
				$this->p_adr_one_region="";
				
				$this->p_adr_one_postalcode=$row['cp_lieu'];	
				$this->p_tel_work=$row['tel_lieu'];	
				$this->p_email=$row['email_lieu'];
...
$requete='SELECT * FROM  apsie_comptes  where account_id='.$id_referent.'';
		$resultat = mysql_query($requete) or die(mysql_error());
...
if($tel_work==NULL)
{
$tel_work='01 46 67 14 18 ';
}
if($mail==NULL)
{
$mail='presta@apsie.org';
}*/
...
$output = str_replace('<<TELORGEVAL>>',$this->p_tel_work, $output);
$output = str_replace('<<SIRET>>','440 826 667 00030', $output);
$output = str_replace('<<MELORGEVAL>>',$this->p_email, $output);
...
$voir = str_replace('<<ACTION>>','Creation d\'entreprise', $voir);
...
function imprimer_totalite($id_beneficiaire,$id_referent)
	...
	$requete='SELECT * FROM  egw_contact  where id_ben='.$id_beneficiaire.'';
		...
		$requete='SELECT * FROM  apsie_jqcalendar  where id_presta='.$this->id_presta.'';
		$resultat = mysql_query($requete) or die(mysql_error());
		while($row = mysql_fetch_array($resultat))
		{
			$cal_category=$row['id_lieu'];
...
$worksheet->write("B5",'P�le emploi : Gestion des bilans de prestation',$heading_pole);
...
$worksheet->write("B10",'N� March�',$heading_l1);
...
$worksheet->write("B12",'N� de d�partement de r�sidence du DE',$heading_l1);
...
$worksheet->write("B15",'Date de d�marrage',$heading_l2);
...
$worksheet->write("B18",'Date �ventuelle d\'abandon',$heading_l1);
