...
if($_REQUEST['select']!='initialize')
{
	...
	if(array_key_exists('select2',$_REQUEST)){
		$xml.=ArrayToXML($_REQUEST['select2']($db),$_REQUEST['node2']);
	}
}
...
function prestataires_deprecie($db,$groupesDuUser)
{
	$prestataires = array();
...
function liste_clients_deprecie($db,$groupesDuUser)
{
	$clients = array();
	$table='spid_clients C, spid_clients_relations R';
	$cat_appname='spid';
...
//Fonction qui va permettre de lister les personnes assignées à 
function ticket_assigned_to($db)
{
	...
	foreach($AssignedTo as $id=>$value)
	{
		...
		if(!empty($details['account_status']) && !$accounts->is_expired($details)){
...
//Fonction qui va permettre de lister les utilisateurs demandeur
function ticket_assigned_by_contact($db)
{
	$client_id = (array_key_exists('valeur',$_REQUEST)) ? $_REQUEST['valeur'] : 0;
	$AssignedBy = spid_bo::get_demandeur($client_id);
	// $accounts =& CreateObject('phpgwapi.accounts',-1,'u');
	// $AssignedBy=$accounts->members($valeur);
	// foreach($AssignedBy as $id=>$value)
	// {
...
// YLF
function contract_id($db){
	$valeur = (array_key_exists('valeur',$_REQUEST)) ? $_REQUEST['valeur'] : ' ';
	$table = 'spiclient_contrats';
	$cat_appname = 'spiclient';
	$cols = array('contract_id, contract_title');
	
	// $where = 'contract_client ='.$valeur;

	$join = 'INNER JOIN spiclient WHERE contract_client ='.$valeur.' OR (contract_client = client_parent AND client_id = '.$valeur.')';
	$append = 'GROUP BY contract_id';
	
	// $db->Debug=5;
	$rows = $db->select($table,$cols,$where,__LINE__,__FILE__,false,$append,$cat_appname,0,$join);
	
	foreach($rows as $row)
	{
		$contrats[$row['contract_id']] = $row['contract_title'];
		// $contrats[]=$row['contract_title'];
	}
...
function reponse($db)
{
	$reponse = array();
	$spid_ui = CreateObject('spid.spid_ui');
	if(array_key_exists('valeur',$_REQUEST) && !empty($_REQUEST['valeur'])){
		...
		$where='R.standard_reply_id='.$_REQUEST['valeur'];
		//$db->Debug=5;
		$rows=$db->select($table,$cols,$where,__LINE__,__FILE__,false,$append,$cat_appname);
		foreach($rows as $row){
			$row['canned_content'] .= $spid_ui->signatureDefault();
		...
		$row = array(
			'standard_reply_id' => '',
			'canned_content' => $spid_ui->signatureDefault(),
			'closed_ticket' => false,
			'state_id' => 0,
			'state_name' => 'test',
...
function etats($db){
	...
	$table='spid_etats E';
	$cat_appname='spid';
	$cols=array('E.state_id','E.state_close');
	$where='E.state_id='.((array_key_exists('valeur',$_REQUEST)) ? $_REQUEST['valeur'] : 0);
	$rows=$db->select($table,$cols,$where,__LINE__,__FILE__,false,$append,$cat_appname);
	foreach($rows as $row)
	{
		$etats[$row['state_id']]=$row;
...
function sendEmail($db)
{
	...
	$infos = array();
	$table='spid_factures F LEFT JOIN spid_clients S ON S.client_id=F.client_id,spid_clients_relations R,spid_clients C';
	$cat_appname='spid';
	$cols=array
	(
		'R.operation_code',
