...
{ 
	...
	// SPIREA-YLF - 09/2016
	this.etats = function (valeur)
	{
		var self=this;
		self.envoi='select=etats';
		self.envoi+='&valeur='+valeur;
		self.envoi+='&node=etats';
		self.xhr.onreadystatechange = function() 
		{
			if(self.xhr.readyState==4 && self.xhr.status==200){
				self.traitementEtatsXML('etat');
			}
			else
			{
				//Chargement en cours
	...
	this.traitementResponseXML = function(node)
	{
		...
		// oEditor.SetHTML("");
		var oEditor = document.getElementById("exec[reply_content]");
		if(count==1)
		{
			message=this.docXML.getElementsByTagName("canned_content")[0].firstChild.nodeValue;
			CKEDITOR.instances["exec[reply_content]"].setData(message);
	...
	this.traitementEtatsXML = function(node)
	{
		this.docXML = this.xhr.responseXML.documentElement;
		this.cleanXML();
		var verif_node=this.docXML.getElementsByTagName(node);
		...
		if(count>=0)
		{
			for(i = 0; i < count; i++)
			{
				closable = this.docXML.getElementsByTagName("state_close")[i].firstChild.nodeValue;
				element_id = this.docXML.getElementsByTagName("state_id")[i].firstChild.nodeValue;

				this.getId('ticket_closed').disabled = true;
				if(closable == 1){
					this.getId('ticket_closed').disabled = false;
				}
			}
		}		
	}
