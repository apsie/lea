	 ...
	 */	
		...
		if(is_array($reponses) && !empty($reponses)){
			foreach($reponses as $id=>$value){
				...
				$reponses[$id]['canned_content'] = lang('Hello')."\r".$reponses[$id]['canned_content']."\r\r".lang('Best regards')."\r";
			}
		}
		// _debug_array($reponses);

		$reponses[''] = array(
			'standard_reply_id' => 0,
			'canned_content' => 'Bonjour, /n/n Cordialement,'
		);
		return $reponses;
	}
	...
	*/
		...
		if(is_array($historique)){
			foreach($historique as $id=>$value){
				...
				if(!empty($action) && (!empty($avant[$id]) || !empty($value))){
					$add_historique=array(
