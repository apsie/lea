...
*/
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.location_bo.inc.php');	
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.acl_spiclient.inc.php');	

class location_ui extends location_bo
{
	...
	*/
		...
		{
			$default_cols='location_id,location_parent,location_name,location_description,creator_id';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spiclient.location_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
				...
				'order'          	=>	'location_parent',	// IO name of the column to sort after (optional for the sortheaders)
		...
		$content['nm']['template']='spiclient.location.index.rows';
		if(!isset($_GET['view'])){
			$content['nm']['header_right'] = 'spiclient.location.index.right';
		}
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Location Management');	
		//_debug_array($content);
		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.location.index');
		$tpl->exec('spiclient.location_ui.index', $content,$sel_options,$no_button, $content);
	}
	 ...
	 */
		...
		foreach((array)$rows as $id=>$value){
			if(isset($query['view'])){
				$readonlys['edit['.$value['location_id'].']']=true;
				$readonlys['delete['.$value['location_id'].']']=true;
				$readonlys['add['.$value['location_id'].']']=true;
			}
			if($value['location_level']>1){
				$readonlys['add['.$value['location_id'].']']=true;
			}
			$rows[$id]['location_parent_name']=$this->id2name($value['location_parent']);
		}
		//_debug_array($rows);
		//pdf[$row_cont[facture_id]]
		$order = $query['order'];
	...
	*/
		...
		if(is_array($content)){
			...
			switch($button){
				...
				case 'apply':
					//_debug_array($content);
					$msg=$this->add_update_location($content);
					if($button=='save'){
		...
		if(isset($id)){
			if(isset($_GET['location_parent'])){
				$location_parent=$_GET['location_parent'];
			}else{
				$location_parent="";
			}
			$content=array(
			...
			if(empty($id)){
				...
				$content['location_parent']=$location_parent;
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Add location');	
			}else{
				$content+=$this->get_info($id);
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Edit location');	
			}
			$sel_options = array(
				'location_parent'	=>$this->get_location(),
			);
		}
			$content['hidebuttons']=false;
			$content['hideline']=true;
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.location.edit');
		$tpl->exec('spiclient.location_ui.edit', $content,$sel_options,$readonlys,$content,2);
	}
	...
	*/
		if(isset($_GET['id'])){
			...
			$GLOBALS['egw']->redirect_link('/index.php','menuaction=spiclient.location_ui.index');
		}
		$content['hidebuttons']=true;
		$readonlys=$content;
		$readonlys['location_parent']=true;
		$content['hideline']=true;
		$GLOBALS['egw_info']['flags']['app_header'] = lang('View location');		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.location.edit');
		$tpl->exec('spiclient.location_ui.view', $content,$sel_options,$readonlys,$content,2);
	}
