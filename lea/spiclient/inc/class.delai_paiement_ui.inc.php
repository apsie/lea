...
*/
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.delai_paiement_bo.inc.php');	
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.acl_spiclient.inc.php');	

class delai_paiement_ui extends delai_paiement_bo
{
	...
	*/
		...
		{
			$default_cols='delai_id,delai_label,delai_defaut';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spiclient.delai_paiement_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
				...
				'order'          	=>	'delai_id',	// IO name of the column to sort after (optional for the sortheaders)
		...
		$content['nm']['template']='spiclient.delai_paiement.index.rows';
		if(!isset($_GET['view'])){
			$content['nm']['header_right'] = 'spiclient.delai_paiement.index.right';
		}
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Delay payment Management');	
		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.delai_paiement.index');
		$tpl->exec('spiclient.delai_paiement_ui.index', $content,$sel_options,$no_button, $content);
	}
	 ...
	 */
		...
		foreach((array)$rows as $id=>$value){
			if(isset($query['view'])){
				$readonlys['edit['.$value['delai_id'].']']=true;
				$readonlys['delete['.$value['delai_id'].']']=true;
				$readonlys['add['.$value['delai_id'].']']=true;
			}
		}
	...
	*/
		...
		if(isset($id)){
			...
			if(empty($id)){
				...
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Add delay payment');	
			}else{
				$content+=$this->get_info($id);
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Edit delay payment');	
			}
		}
			$content['hidebuttons']=false;
			$content['hideline']=true;
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.delai_paiement.edit');
		$tpl->exec('spiclient.delai_paiement_ui.edit', $content,$sel_options,$readonlys,$content,2);
	}
	...
	*/
		if(isset($_GET['id'])){
			...
			$GLOBALS['egw']->redirect_link('/index.php','menuaction=spiclient.delai_paiement_ui.index');
		}
		...
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.delai_paiement.edit');
		$tpl->exec('spiclient.delai_paiement_ui.view', $content,$sel_options,$readonlys,$content,2);
	}
