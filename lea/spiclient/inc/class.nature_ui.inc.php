...
*/
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.nature_bo.inc.php');	
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.acl_spiclient.inc.php');	

class nature_ui extends nature_bo
{
	...
	*/
		...
		{
			$default_cols='nature_id,nature_label,nature_actif';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spiclient.nature_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
				...
				'order'          	=>	'nature_id',	// IO name of the column to sort after (optional for the sortheaders)
				'sort'           	=>	'ASC',		// IO direction of the sort: 'ASC' or 'DESC'
				'col_filter'     	=>	array(),	// IO array of column-name value pairs (optional for the filterheaders)
				'filter_label'   	=>	lang('Technical nature'),	// I  label for filter    (optional)
		...
		$content['nm']['template']='spiclient.nature.index.rows';
		if(!isset($_GET['view'])){
			$content['nm']['header_right'] = 'spiclient.nature.index.right';
		}
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Technical nature management');	
		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.nature.index');
		$tpl->exec('spiclient.nature_ui.index', $content,$sel_options,$no_button, $content);
	}
	 ...
	 */
		...
		foreach((array)$rows as $id=>$value){
			if(isset($query['view'])){
				$readonlys['edit['.$value['nature_id'].']']=true;
				$readonlys['delete['.$value['nature_id'].']']=true;
				$readonlys['add['.$value['nature_id'].']']=true;
			}
		}
		...
		if($query['filter']){
			$GLOBALS['egw_info']['flags']['app_header'] .= ' - '.lang("Search '%1'",$this->id2name($query['filter']));
		}
		return $this->total;	
    }
	...
	*/
		...
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.nature.edit');
		$tpl->exec('spiclient.nature_ui.edit', $content,$sel_options,$readonlys,$content,2);
	}
	...
	*/
		if(isset($_GET['id'])){
			...
			$GLOBALS['egw']->redirect_link('/index.php','menuaction=spiclient.nature_ui.index');
		}
		$content['hidebuttons']=true;
		$readonlys=$content;
		$content['hideline']=true;
		$GLOBALS['egw_info']['flags']['app_header'] = lang('View technical nature');		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.nature.edit');
		$tpl->exec('spiclient.nature_ui.view', $content,$sel_options,$readonlys,$content,2);
	}
