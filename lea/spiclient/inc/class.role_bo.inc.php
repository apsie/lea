...
*/
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.role_so.inc.php');	
...
class role_bo extends role_so 
{
	...
	function get_info($id){
		...
		* @return string
		*/
		$info=$this->search(array('role_id'=>$id),false);
		return $info[0];
	}
	
	function add_update_role($content=null){
		/**
		* Routine permettant de créer/modifier (si le secteur existe déjà) le secteur actuel, à partir des arguments passés dans $content. $content doit contenir un champ role_id pour définir le secteur à créer/mettre à jour
		*
