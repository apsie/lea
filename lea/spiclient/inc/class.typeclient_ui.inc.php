...
*/
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.typeclient_bo.inc.php');	
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.acl_spiclient.inc.php');	

class typeclient_ui extends typeclient_bo
{
	...
	*
	* Pour supprimer des index, assigner un tableau d'identifiants de typeclient à supprimer à $content['nm']['rows']['delete']
	*
	...
	*/
		...
		{
			$default_cols='type_client_id,typeclient_parent,typeclient_pays,typeclient_label,creator_id';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spiclient.typeclient_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
				...
				'order'          	=>	'type_client_id',	// IO name of the column to sort after (optional for the sortheaders)
				'sort'           	=>	'ASC',		// IO direction of the sort: 'ASC' or 'DESC'
				'col_filter'     	=>	array(),	// IO array of column-name value pairs (optional for the filterheaders)
				// 'filter_label'   	=>	lang('Client type'),	// I  label for filter    (optional)
		...
		$content['nm']['template']='spiclient.typeclient.index.rows';
		if(!isset($_GET['view'])){
			$content['nm']['header_right'] = 'spiclient.typeclient.index.right';
		}
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Client type management');
		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.typeclient.index');
		$tpl->exec('spiclient.typeclient_ui.index', $content,$sel_options,$no_button, $content);
	}

	function get_rows($query,&$rows,&$readonlys){
	/**
	 * Récupère et filtre les typeclients
	 *
	 ...
	 */
		...
		foreach((array)$rows as $id=>$value){
			if(isset($query['view'])){
				$readonlys['edit['.$value['type_client_id'].']']=true;
				$readonlys['delete['.$value['type_client_id'].']']=true;
				$readonlys['add['.$value['type_client_id'].']']=true;
			}
		}
	...
	*
	* $content contient 2 index : 'button' et, au choix save,apply,cancel
	*
	* Le contenu à visualiser peut se faire via $_GET['id'] ($_GET['typeclient_parent'] permets d'appliquer un filtre)
	*
	...
	*/
		...
		if(is_array($content)){
			...
			switch($button){
				...
				case 'apply':
					...
					$msg=$this->add_update_typeclient($content);
					if($button=='save'){
		...
		if(isset($id)){
			...
			if(empty($id)){
				...
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Add Client type');	
			}else{
				$content+=$this->get_info($id);
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Edit Client type');	
			}
		}
			$content['hidebuttons']=false;
			$content['hideline']=true;
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.typeclient.edit');
		$tpl->exec('spiclient.typeclient_ui.edit', $content,$sel_options,$readonlys,$content,2);
	}
	
	function view(){
	/**
	* Affiche une typeclient (en lecture seule)
	*
	...
	*/
		if(isset($_GET['id'])){
			...
			$content=$this->get_info($id);
			$sel_options = array(
				'typeclient_parent'	=> $this->get_typeclient_parent(),
			);
		}else{
			$GLOBALS['egw']->redirect_link('/index.php','menuaction=spiclient.typeclient_ui.index');
		}
		$content['hidebuttons']=true;
		$readonlys=$content;
		$readonlys['typeclient_parent']=true;
		$content['hideline']=true;
		$GLOBALS['egw_info']['flags']['app_header'] = lang('View Client type');		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.typeclient.edit');
		$tpl->exec('spiclient.typeclient_ui.view', $content,$sel_options,$readonlys,$content,2);
	}
