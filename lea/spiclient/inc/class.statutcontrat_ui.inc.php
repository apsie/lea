...
*/
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.statutcontrat_bo.inc.php');	
require_once(EGW_INCLUDE_ROOT. '/spiclient/inc/class.acl_spiclient.inc.php');	

class statutcontrat_ui extends statutcontrat_bo
{
	...
	*/
		...
		{
			$default_cols='status_id,status_label,status_description';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spiclient.statutcontrat_ui.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
		...
		$content['nm']['template']='spiclient.statutcontrat.index.rows';
		if(!isset($_GET['view'])){
			$content['nm']['header_right'] = 'spiclient.statutcontrat.index.right';
		}
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Contract type Management');	
		
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.statutcontrat.index');
		$tpl->exec('spiclient.statutcontrat_ui.index', $content,$sel_options,$no_button, $content);
	}
	...
	*/
		...
		if(is_array($content)){
			...
			switch($button){
				case 'save':
				case 'apply':
					$msg=$this->add_update_type_contrat($content);
					if($button=='save'){
		...
		if(isset($id)){
			...
			if(empty($id)){
				...
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Add contract type');	
			}else{
				$content+=$this->get_info($id);
				$GLOBALS['egw_info']['flags']['app_header'] = lang('Edit contract type');	
			}
		}
			$content['hidebuttons']=false;
			$content['hideline']=true;
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.statutcontrat.edit');
		$tpl->exec('spiclient.statutcontrat_ui.edit', $content,$sel_options,$readonlys,$content,2);
	}
	...
	*/
		if(isset($_GET['id'])){
			...
			$GLOBALS['egw']->redirect_link('/index.php','menuaction=spiclient.statutcontrat_ui.index');
		}
		...
		$tpl = CreateObject('etemplate.etemplate', 'spiclient.statutcontrat.edit');
		$tpl->exec('spiclient.statutcontrat_ui.view', $content,$sel_options,$readonlys,$content,2);
	}
