 ...
 * @version $Id: class.phpbrain_bo.inc.php 43975 2013-10-03 09:23:18Z ralfbecker $
 */
...
class phpbrain_bo extends sokb
{
	...
	**/
	...
	static function get_files($art_id)
	{
		...
		//_debug_array(array('id'=>$art_id,'file'=>$files));
		return $files;
	}

	/**
	* Returns article basic info with linked files
	*	id, subject, body, attachments
	* The returned article does include the files
	*
	* @author	Klaus Leithoff
	* @access	public
	* @param	mixed		$art_id		article id or array with article_id and mime-type
	* @param	string		$mimetype 	html or text
	* @return	array/boolean array with article, null if not found
	**/
	function retrieve_article($art_id, $mimetype='html')
	{
		//error_log(__METHOD__.__LINE__.array2string($art_id));
		if (is_array($art_id))
		{
			$art_id = $art_id[0];
		...
		if (!$this->check_permission($this->read_right | $this->publish_right))
		{
			return array('id'=>$art_id, 'subject'=>lang('You have not the proper permissions to do that'), 'mimetype'=>$mimetype, 'body'=>lang('You have not the proper permissions to do that'));
		}
		if ($files = phpbrain_bo::get_files($art_id))
		{
			//error_log(__METHOD__.__LINE__.array2string($files));
			foreach($files as $f)
			{
				$article['attachments'][] = str_replace('/webdav.php','vfs://default',egw_vfs::download_url(egw_link::vfs_path('phpbrain',$art_id,$f['file'])));
			}
		}
		return array('id'=>$art_id, 'subject'=>$article['title'], 'mimetype'=>$mimetype, 'body'=>$article['text'],'attachments'=>$article['attachments']);
	}
