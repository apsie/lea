...
class spisession_bo extends spisession_so{
	 ...
	 */
		...
		// Lien vers le cours
		$info['crs_link'] = '<a href="http://unil.test.spirea.fr/index.php?menuaction=spisession.course_ui.edit&amp;id='.$info['ses_crs'].'" onclick="window.open(this,this.target,\'width=990,height=600,location=no,menubar=no,toolbar=no,scrollbars=yes,status=yes\'); return false;">'.lang('See course').'</a>';
	 ...
	 */
		...
		if(!empty($query['filter_reg'])){
			switch ($query['filter_reg']) {
				case 'registered':
					...
					$join .= 'INNER JOIN spisession_registration ON ses_id = reg_ses WHERE reg_contact = '.$GLOBALS['egw_info']['user']['person_id'].' AND reg_status <> '.$this->obj_config['rejected_reg_status'].' AND reg_status <> '.$this->obj_config['pending_reg_status'].' AND reg_role = '.$this->obj_config['student_role'];
					break;
				case 'waiting_list':
					// Participant + liste d'attente
					$join .= 'INNER JOIN spisession_registration ON ses_id = reg_ses WHERE reg_contact = '.$GLOBALS['egw_info']['user']['person_id'].' AND reg_status = '.$this->obj_config['pending_reg_status'].' AND reg_role = '.$this->obj_config['student_role'];
					break;
				case 'reg_possible':
					// Inscription ouverte + nb_participant < max_participant + pas deja inscrit
					$join .= 'WHERE ses_start_reg < '.time().' AND ses_end_reg > '.time().' AND ses_id NOT IN (SELECT reg_ses FROM spisession_registration WHERE reg_contact = '.$GLOBALS['egw_info']['user']['person_id'].' AND reg_role = '.$this->obj_config['student_role'].' AND reg_status <> '.$this->obj_config['rejected_reg_status'].')';
					break;
		...
		if($GLOBALS['egw_info']['user']['SpiSessionLevel'] == 1 && !empty($this->obj_config['archived_ses_status'])){
			if(strpos($join, 'WHERE') !== FALSE){
		...
		foreach((array)$rows as $id=>$value){
			...
			$reg = $this->so_reg->search(array('reg_contact'=>$GLOBALS['egw_info']['user']['person_id'],'reg_ses'=>$rows[$id]['ses_id']),false);
			if(is_array($reg)){
	...
	function truncate_word($string, $limit=20, $break="-", $pad="...") { 
	/**
