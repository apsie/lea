...
echo'<select style="width:250px" name="dispositif">';
	...
	function return_nom_projet($id_ben)
	{
       ...
       for($i=0;$i<count($result);$i++)
	   {
		   ...
		   echo'<h2><img src="../images/32x32/category.png" /> <a>PROJET</a> : <a href="javascript:voirdiv(\'div_projet\')">'.$result[$i]['intitule_projet'].'  ( '.$result[$i]['statut'].' - '.$this->nbr_presta_by_projet($result[$i]['id_projet']).' prestations )</a> <input  onclick="window.open(\'../../../Projet1_1/details.php?domain=default&id_projet='.$result[$i]['id_projet'].'\',\'Projet\',\'type=fullWindow,fullscreen,scrollbars=yes\');" type="button" value="Modifier"/></h2>';
	...
	function link_new_prescripteur($id_ben,$code_safir,$nom,$prenom,$civilite,$tel_b,$tel_p,$email_b,$email_d,$fonction)
{
	...
	function return_contact_prescripteur($id_owner,$code_safir,$nom,$prenom,$civilite,$tel_bureau,$fax_pro,$email_pro,$email_perso,$fonction)
	{
...
<table width="100%">
  ...
  if($result['id_contact_prescripteur']!=0)
  {
	  ...
	  $result2 = $this->db->fetchRow($requete2);
		  ...
		  {
                  $idOrganisation = explode(',',$result2['id_organisation']);
	  $requete3='select * from '.$this->table_organisation.' where id_organisation='.$idOrganisation[0].'';
...
<table  width="100%">
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="8%">Date</td><td width="20%" height="21" class="titre2">Intitule du poste</td><td width="9%" >Service</td><td width="4%" >Statut</td><td width="4%" >Identifiant</td><td width="5%" >Contrat</td><td width="7%" >Contrat aide</td><td width="4%">Type.R</td><td width=8%">Remuneration</td><td width="8%">Qualifiquation</td><td width="3%">T.</td><td width="7%">Deplacement</td><td width="15%">Organisme</td><td ></td><td></td></tr>';
		...
		for($i=0;$i<count($result);$i++)
		{
			...
			$requete='SELECT * FROM  egw_organisation  WHERE nom_organisme = "'.$result[$i]['organisme'].'" limit 1';
...
<table style="border:1px dotted #CCC">
  <tr  style="font-weight:bolder" ><td width="300px"  class="titre2">Entretien / objectif</td><td >Date de prevu</td><td >heure</td></tr>';
		...
		$requete2='SELECT * FROM  egw_cal_dates  where cal_id=0 '.$req2.' order by cal_start asc limit 6';
	...
	function nbre_rdv_plan($id_presta,$choix)
	{
		...
		$requete='SELECT cal_id FROM  egw_cal_user where (cal_id=0 '.$req.' ) and cal_user_type="u" and cal_status!="R" order by cal_id desc';
	//echo $requete;
		$resultat = mysql_query($requete) or die(mysql_error());
...
<table width=100%>
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="200px" height="21" class="titre2">Prestation</td><td width="200px" >ID_Prestation</td><td width="200px">Conseiller</td><td width="200px">Date de debut</td><td width="200px">Date de fin</td><td width="200px">Lieu</td><td width="100px">Statut</td><td width="20px" ></td><td width="100px" ></td></tr>';
		...
		$requete='	SELECT p.*,tp.libelle as presta FROM  egw_prestation p 
					left join egw_dispositif d on d.id_dispositif = p.id_dispositif
					left join egw_type_prestation tp on tp.id = d.id_type_prestation
				 	where id_ben="'.$choix.'" and id_projet="'.$id_projet.'" order by date_debut desc';
		//die($requete);
	$result=$this->db->fetchAll($requete);
...
<table  width="100%">
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="19%" height="21" class="titre2">Intitule du rendez-vous</td><td width="18%" >Date de debut</td><td width="17%">Date de fin</td><td width="17%">Lieu</td><td width="12%">Participants</td><td width="5%">Statut</td><td  width="10%">
		...
		$requete='SELECT * from egw_cal where cal_title like "%'.$retour[1].' '.$retour[2].'%" and  id_presta!='.$id_presta.' and id_presta=0 order by cal_id asc ';
		//echo $requete;
		...
		{
			$cal_id[]=$resultat[$i]['cal_id'];
		
	
			$cal_title[]=$resultat[$i]['cal_title'];
			$cal_category[]=$resultat[$i]['cal_category'];
		...
		for($z=0;$z<count($cal_id);$z++)
		{
			...
			{
				$requete4='SELECT cat_name FROM  egw_categories  where cat_id like "%'.$cal_category[$z].'%" ';
				
		$resultat=$this->db->fetchAll($requete4);
		for($i=0;$i<count($resultat);$i++)
		{
			$cat_name=$resultat[$i]['cat_name'];
			
		
			
		}
			}
				$requete4='SELECT cal_user_id ,cal_status FROM  egw_cal_user  where cal_user_type ="u" and  cal_id = '.$cal_id[$z].' ';
		...
		for($i=0;$i<count($resultat);$i++)
		{
		    ...
		    $cal_status=$resultat[$i]['cal_status'];
				$requete6='SELECT ad.* FROM  egw_accounts a left join egw_addressbook ad on ad.account_id = a.account_id where a.account_id='.$cal_user_id.' ';
		$resultat=$this->db->fetchAll($requete6);
		for($i=0;$i<count($resultat);$i++)
		{
			$account_firstname=$resultat[$i]['n_family'];
			$account_lastname=$resultat[$i]['n_given'];
			
		}
			
		}
		
				$requete4='SELECT cal_end,cal_start FROM  egw_cal_dates  where cal_id = '.$cal_id[$z].' ';
				
		$resultat=$this->db->fetchAll($requete4);
		for($i=0;$i<count($resultat);$i++)
		{
			$cal_end=$resultat[$i]['cal_end'];
			$cal_start=$resultat[$i]['cal_start'];
		...
		echo'<form action="panel.php" method="get"><tr><td>'.$cal_title[$z].'</td><td width="18%" >'.$cal_start_.' '.$cal_start_m.' </td><td width="17%">'.$cal_end_.' '.$cal_end_m.'</td><td width="17%"><font color="#FF3300">';
		...
		echo'</font></td><td width="12%">'.$account_lastname.' '.$account_firstname.'</td><td>'.$cal_status.'</td><td><input type="hidden" name="id_rdv" value="'.$cal_id[$z].'" /><input type="hidden" name="choix" value="'.$choix.'" /><input type="hidden" name="id_presta" value="'.$id_presta.'" /><input type="hidden" name="cal_status" value="'.$cal_status.'" /><input type="hidden" name="display_eval" value="block" /><input type="submit" value="Lier a cette prestation" name="lier" /></td></tr></form>';
  ...
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="19%" height="21" class="titre2">Intitule du rendez-vous</td><td width="18%" >Date de debut</td><td width="17%">Date de fin</td><td width="8%">Lieu</td><td width="12%">Participants</td><td width="5%">Statut</td><td width="8%">Compte rendu</td><td  width="10%"></td></tr>';
		...
		for($i=0;$i<count($resultat);$i++)
		{
			...
			$account_lastname=$resultat[$i]['n_given'];
				...
				$requete4='SELECT cal_end,cal_start FROM  egw_cal_dates  where cal_id = '.$cal_id[$z].' order by cal_start asc ';
		...
		{
			$compte_rendu='<a href="../../../Compte_rendu1_0/impression.php?domain=default&cal_id='.$cal_id[$z].'&id_employee='.$id_employee.'&id_projet='.$this->get_id_projet($id_presta).'&id_presta='.$id_presta.'&id_ben='.$choix.'&id_compte_rendu='.$resultat5['id_compte_rendu'].'"><img title="Edition du compte rendu" border="0" src="../images/icons/page_white_word.png" /></a>';
		}
			
			elseif($resultat5['id_compte_rendu']==NULL and $cal_status!='R')
			{$compte_rendu='<a onclick="window.open(\'../../../Compte_rendu1_0/index.php?domain=default&cal_id='.$cal_id[$z].'&id_employee='.$id_employee.'&id_projet='.$this->get_id_projet($id_presta).'&id_presta='.$id_presta.'&id_ben='.$choix.'\', \'Compte rendu\', \'left=0,top=0,width=1050,height=600,scrollbars=1\')" href="javascript::void()" ><img title="Redaction du compte rendu" border="0" src="../images/icons/page_white_paintbrush.png" /></a>';
			}
		...
		echo'</font></td><td width="12%">'.$account_lastname.' '.$account_firstname.'</td><td><input type="hidden" name="id_presta" value="'.$id_presta.'" /><input type="hidden" name="display_eval" value="block" /><input type="hidden" name="id_rdv" value="'.$cal_id[$z].'" /><input type="hidden" name="choix" value="'.$choix.'" /><select onchange="submit()" name="stat" style="width:40px;"><option>'.$cal_status.'</option><option value="A">A</option><option value="R">R</option><option value="U">U</option></select></td><td align="center">'.$compte_rendu.'</td><td><a href="panel.php?choix='.$choix.'&id_presta='.$id_presta.'&display_eval=block&id_rdv_unlink='.$cal_id[$z].'" ><img border="0" title="Ne pas lier ce rdv a cette prestation" src="../images/icons/disconnect.png"  /></a></td></tr></form>';
   ...
   $account_lastname=$result['account_lastname'];
	...
	echo '<div align="center"><form action="../presentation/nouveau_beneficiaire.php" method="get"><table><tr><td><input type="hidden" name="conseiller_id" value="'.$conseiller_id.'"/><input type="hidden" name="option_ben" value="nouveau"/><strong>Confirmer les options de <font color=red>'.$account_firstname.' '.$account_lastname.'</font> a partir du '.$date_inscription.' </strong> </td></tr></table><br/>
		...
		if($lieu==1)
		{
			 if($conseiller_id!=NULL)
				...
				$requete4='SELECT egw_cal_user.cal_id,egw_cal.cal_id,egw_cal.cal_title,egw_cal_user.cal_user_id FROM  egw_cal,egw_cal_user where egw_cal_user.cal_id=egw_cal.cal_id and cal_title like "%Option%"  and cal_user_id='.$conseiller_id.' order by egw_cal.cal_id desc';
				
				}
				else
				{
				$requete4='SELECT egw_cal_user.cal_id,egw_cal.cal_id,egw_cal.cal_title,egw_cal_user.cal_user_id FROM  egw_cal,egw_cal_user  where cal_title like "%Option%" and egw_cal_user.cal_id=egw_cal.cal_id order by egw_cal.cal_id desc ';
				}
		...
		else
		{
			 ...
			 if($conseiller_id!=NULL)
 				{
					$requete4='SELECT egw_cal_user.cal_id,egw_cal.cal_id,egw_cal.cal_title,egw_cal_user.cal_user_id FROM  egw_cal,egw_cal_user where egw_cal_user.cal_id=egw_cal.cal_id and cal_title like "%Option_'.$lieu.'"  and cal_user_id='.$conseiller_id.' order by egw_cal.cal_id desc';
		
				}
				else
				{
				$requete4='SELECT egw_cal_user.cal_id,egw_cal.cal_id,egw_cal.cal_title,egw_cal_user.cal_user_id FROM  egw_cal,egw_cal_user  where cal_title like "%Option_'.$lieu.'" and egw_cal_user.cal_id=egw_cal.cal_id order by egw_cal.cal_id desc';
				}
		}
	...
	for($i=0;$i<count($cal_id);$i++)
				...
				for($i=0;$i<count($result6);$i++)
		{
			...
			$requete5='SELECT * FROM  egw_accounts  WHERE account_id='.$result8['cal_user_id'].' limit 1';
	...
	function link_beneficiaire_calendar($id_recup,$opt,$nb,$n_given,$n_family,$tel_work,$tel_cell,$lc,$pole_id,$nom_p,$prenom_p,$tel_home,$code_safir,$tel_contact,$tel_contact2,$date_deb,$id_presta)
	{
		...
		$requete='SELECT * FROM '.$this->table_cal.'  where cal_id='.$opt.'';
		
		$result=$this->db->fetchRow($requete);
		
			$cal_title=$result['cal_title'];
			$ti = explode("_", $cal_title);
			$ti[0];
			$ti[1];
			
		
			$requete2='SELECT adresse_ligne_1,cp,ville FROM '.$this->table_organisation.' WHERE nom_organisme like "%APSIE '.$ti[3].'" order by id_organisation desc limit 1';
	
		$result2=$this->db->fetchRow($requete2);	
		
		
		$requete3='SELECT cal_start FROM  '.$this->table_cal_dates.'  where cal_id='.$opt.'';
		$result3=$this->db->fetchRow($requete3);
		...
		$data = array ('id_presta'=>$id_presta , 'cal_description'=>' 
Tel Bureau : '.$tel_work.'
...
Prestation debut : '.date('d/m/Y à H:i',$result3['cal_start']).'' ,'cal_title'=>$ti[0].'_'.$ti[1].'_'.$n_family.' '.$n_given);

$this->db->update($this->table_cal,$data,'cal_id='.$opt.'');
...
function chercher_options($date_choisi,$selection,$plage1,$plage2,$duree,$conseiller_id,$lieu,$nombre,$jour)
    ...
    $result=$this->db->fetchRow($requete);
		...
		echo'<hr /><form name="test" action="poser.php" method="get"><table><tr><td><font color=red>'.$result['account_firstname'].' '.$result['account_lastname'].' </font>pour prestations</td><td><select name="prestation"> <option value="OEM77">OEM77</option><option value="OEM94">OEM94</option><option value="TVE77">TVE77</option><option value="CAP77">CAP77</option><option value="EPCE77">EPCE77</option><option value="OPCRE77">OPCRE77</option><option value="NACRE1">NACRE1</option><option value="NACRE3">NACRE3</option><option value="PDI92">PDI92</option><option value="BC">BC</option><option value="VAE">VAE</option><option value="EPI_BP">EPI_BP</option></select> a partir du '.$date_choisi.'</td><td><input  name="lieu" type="hidden" value="'.$lieu.'" /><input type="hidden" name="conseiller_id" value='.$conseiller_id.' /><input type="hidden" name="date_choisi" value='.$date_choisi.' /><input type="hidden" name="conseiller" value="'.$result['account_firstname'].' '.$result['account_lastname'].'" /></td></tr></table>';
		...
		$requete3='SELECT cal_end,cal_start FROM  egw_cal_dates  where (cal_start>'.$base.') and (cal_id=0 '.$req.')   order by cal_start desc';
	...
	foreach($d16 as $maCle=>$maValeur)
{
   ...
   echo '<table ><tr><td width="100">'.$_j6[$maCle].'</td><td> '.date(' d/m/Y | H:00',$time16[$maCle]).'</td><td >'.date(' d/m/Y | H:00',$time26[$maCle]).'</td><td ><input name="pose[]" value="'.$time16[$maCle].'-'.$time26[$maCle].'" type="checkbox" /></td></tr></table>';
	...
	/*	echo'<hr /><form name="test" action="poser_rdv.php" method="get"><table><tr><td><font color=red>'.$account_firstname.' '. $account_lastname.' </font>pour prestations</td><td><select name="prestation"> <option value="OEM77">OEM77</option><option value="OEM94">OEM94</option><option value="TVE77">TVE77</option><option value="CAP77">CAP77</option><option value="EPCE77">EPCE77</option><option value="OPCRE77">OPCRE77</option><option value="NACRE1">NACRE1</option><option value="NACRE3">NACRE3</option><option value="PDI92">PDI92</option><option value="BC">BC</option><option value="VAE">VAE</option><option value="EPI_BP">EPI_BP</option></select> a partir du '.$date_choisi.'</td><td><input  name="lieu" type="hidden" value="'.$lieu.'" /><input type="hidden" name="conseiller_id" value='.$conseiller_id.' /><input type="hidden" name="date_choisi" value='.$date_choisi.' /><input type="hidden" name="conseiller" value="'.$account_firstname.' '.$account_lastname.'" /></td></tr></table>';*/
...
/*	if($z>$nombre)
    ...
    $data = array('cal_id' => $id , 'cal_start'=> $cal_start , 'cal_end'=> $cal_end) ;
	...
	function inserer_cal_user($id,$conseiller_id)
	{
	   ...
	   $data = array('cal_id' => $id , 'cal_recur_date'=> 0 , 'cal_user_type'=>'u' , 'cal_user_id'=>$conseiller_id,'cal_status'=>'U','cal_quantity'=>'1') ;
	...
	$data = array('cal_owner' => $conseiller_id, 'cal_category'=> $result2['cat_id'] , 'cal_priority'=>2,'cal_public'=>1,'cal_title'=>$titre,'cal_reference'=>0,'id_presta'=>$id_presta) ;			
	$this->db->insert($this->table_cal,$data);
	
			
	
	
	$requete3='SELECT * FROM  egw_cal order by cal_id desc limit 1';
	$result3=$this->db->fetchRow($requete3);
	return $result3['cal_id'];
...
$requete = "insert into phpgw_p_hours value ('', '$employee', '$project_id', '$activity_id', '$entry_date', '$start_date', '$end_date', '$remark', '$minutes', '$status', '$hours_descr', '$dstatus', '$pro_parent', '$pro_main', '$billable', '$km_distance', '$t_journey', '$cost_id')";
	...
	function rdv_statut($beneficiaire,$statut,$id_presta="")
	{
		if($id_presta=="")
		{
		 $requete='SELECT cal_id,cal_category FROM  egw_cal   where cal_title like "%EPC93_'.$beneficiaire.'%" order by cal_id desc limit 1';
		}
		else
		{
		$requete='SELECT cal_id,cal_category FROM  egw_cal   where id_presta='.$id_presta.' order by cal_id desc limit 1';
		}
		$result = $this->db->fetchRow($requete);
	...
	$this->db->update($this->table_cal_user,$data,'cal_id='.$cal_id);
...
if($statut=="R")
{
	...
	function rdv_change_statut($id_rdv,$statut,$id_presta)
	{
		 $requete = 'Update egw_cal_user set cal_status="'.$statut.'"  where  cal_id='.$id_rdv.'';
			...
			if($id_presta=="")
		{
		 $requete='SELECT cal_id,cal_category FROM  egw_cal   where cal_id='.$id_rdv.' order by cal_id desc limit 1';
		}
		else
		{
		$requete='SELECT cal_id,cal_category FROM  egw_cal   where id_presta='.$id_presta.' cal_id='.$id_rdv.'  order by cal_id desc limit 1';
		}
		$result = $this->db->fetchRow($requete);
...
if($statut=="R")
{
 $requete = 'Update egw_cal set cal_category="'.$cal_category.','.$this->cal_id_rejete.'"  where  cal_id='.$cal_id.'';
	...
	$requete='SELECT * FROM  egw_accounts  where  account_id='.$account_id.'';
		$result=$this->db->fetchRow($requete);
			...
			$account_firstname=$result['account_lid'];
			$account_lastname=$result['account_lastname'];
	...
	{
		$requete='SELECT cal_category FROM  egw_cal  where id_presta='.$id_presta.' order by cal_id asc';
...
<table  width="100%">
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="10%">Statut</td><td width="10%">Type de la formation</td><td width="14%">Niveau</td><td  width="12%" height="21" class="titre2">Intitule du diplome</td><td width="6%" >Resultat</td><td width="10%">Date debut</td><td width="10%">Date fin</td><td width="12%">Organisme formation</td><td width="12%">Organisme certification</td><td width="16%"></td></tr>';
		...
		for($i=0;$i<count($result);$i++)
		{
			...
			$requete='SELECT * FROM  egw_organisation  WHERE nom_organisme = "'.$result[$i]['organisme_formation'].'" limit 1';
			$result2=$this->db->fetchRow($requete);
			$requete='SELECT * FROM  egw_organisation  WHERE nom_organisme = "'.$result[$i]['organisme_certification'].'" limit 1';
...
<table  width="100%">
  <tr style="background:url(./images/level2Bg.gif) repeat-x; background-position:bottom; height:25px; " ><td width="15%">Nom commercial</td><td width="21%">Activite principale</td><td width="14%">Forme juridique</td><td  width="18%">Secteur d\'activite</td><td width="6%" >Code postal</td><td width="10%">Ville</td><td width="7%">Statut</td></tr>';
		...
		$requete="SELECT * FROM  ".$this->table_projet_organisation."  WHERE id_ben=$id_ben and id_projet=$id_projet order by id_resacc desc";
		...
		for($i=0;$i<count($result);$i++)
		{
			...
			echo'<tr><td>'.addslashes($result[$i]['nom_commercial']).'</td><td width="10%">'. utf8_encode($result[$i]['activite_principale']).'</td><td width="14%">'.$result[$i]['forme_juridique'].'</td><td width="18%" height="21" class="titre2">'.utf8_encode($result[$i]['secteur_activite']).'</td><td >'.$result[$i]['cp'].'</td><td >'.$result[$i]['ville'].'</td><td >'.$result[$i]['statut'].'</td></tr>';
		}
	...
	{
		$org_certification=str_replace("é",'e',$org_certification);
		$org_certification=str_replace("é",'e',$org_certification);
		$org_formation=str_replace("é",'e',$org_formation);
		$org_formation=str_replace("é",'e',$org_formation);
...
function inserer_resacc($id_ben,$id_owner,$id_modifier,$id_projet,$nom_commercial,$activite_principale,$raison_sociale,$type_adresse,$adresse_ligne_1,$adresse_ligne_2,$adresse_ligne_3,$cp,$ville,$date_immat,$date_debut_activite,$forme_juridique,$siret,$secteur_activite,$dirigeant,$implantation,$regime_imposition,$regime_tva,$regime_fiscal,$regime_social_dirigeant,$statut)
	...
	{
		$org_employeur=str_replace("é",'e',$org_employeur);
		$org_employeur=str_replace("é",'e',$org_employeur);
...
<table width=100%>
  <tr style="background:url(./images/ligne_relance.gif) repeat-x; background-position:bottom; height:25px; " ><td width="100px" >ID_relance</td><td width="100px" height="21" class="titre2">Prestation</td><td width="100px" >ID_Prestation</td><td width="150px">Relanceur</td><td width="200px">Date de la relance</td><td width="100px">Type</td><td width="300px">Motif</td><td width="300px">Resultat</td><td width="200px" >Prochain contact</td><td width="20px" >Le</td></tr>';
	...
	$row2=$this->db->fetchRow($requete);
		...
		if($type[$i]=="phone")
{
	$type[$i]='<img title="Appel telephonique" src="../../images/phone.png" />';
}
