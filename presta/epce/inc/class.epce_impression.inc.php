...
public $db_host ="localhost";
	...
	public $date_debut_epce_form;
	public $date_fin_epce;
	public $p_org_name="APSIE";
	public $p_adr_one_street ="27 rue de rouen";
	public $p_adr_one_locality="COURBEVOIE";
	public $p_adr_one_region="IDF";
	public $p_adr_one_postalcode ="92400";	
	public $p_tel_work="01 46 67 14 18";	
	...
	public $tmp_fin;
	public $imprimer;
				public $siret_form="44082666700030";
				public $code_postal_ben;
	
	public $id_presta;
	public $email_bureau_prescripteur="presta@apsie.org";
	public $n_marche;
	public $n_lot;
...
//ORG PRESCRIPTEUR
 ...
 $resultat = mysql_query($requete) or die(mysql_error());
  ...
  if($id_contact_prescripteur!=0)
  {
	  ...
	  $requete2='select * from egw_contact where id_ben='.$id_contact_prescripteur.'';
	  ...
	  $requete3='select * from egw_organisation where id_organisation="'.$id_organisation.'"';
         // die($requete3);
	   $resultat = mysql_query($requete3) or die(mysql_error());
	  while($row = mysql_fetch_array($resultat))
		{
		...
		if($row['email']!=NULL)
		{
		$this->email_bureau_prescripteur=$row['email'];
		}
...
//RDV
		...
		$this->cal_5 = $cal_start_ben[4];
		$this->cal_6 = $cal_start_ben[5];
...
//RECUPERATION BILAN
$requete='SELECT code_rome,avis1 FROM  egw_epce_bilan_avis  where id_presta='.$id_presta.'';
...
$requete='SELECT lettre_de_commande,p.date_debut,p.date_fin,d.numero_marche,d.numero_lot  FROM  egw_prestation p left join egw_dispositif d on d.id_dispositif = p.id_dispositif  where p.id_presta='.$id_presta.'';
		$resultat = mysql_query($requete) or die(mysql_error());
		while($row = mysql_fetch_array($resultat))
		{
		$this->lalettre=$row['lettre_de_commande'];
		$this->date_debut_epce=date('dmy',$row['date_debut']);
		$this->date_debut_epce_form=date('d/m/Y',$row['date_debut']);
		$this->date_fin_epce=date('dmy',$row['date_fin']);
		$this->tmp_fin=$row['date_fin'];
		$this->n_marche=$row['numero_marche'];
		$this->n_lot=$row['numero_lot'];
		}
...
$requete='SELECT ad.n_given,ad.n_family,ad.contact_email FROM  egw_accounts ac 
left join egw_addressbook ad on ad.account_id = ac.account_id
 where ac.account_id='.$id_referent.'';
		$resultat = mysql_query($requete) or die(mysql_error());
		
		while($row = mysql_fetch_array($resultat))
		{
			$this->prenom_referent=$row['n_given'];
			$this->nom_referent=$row['n_family'];
			$this->email_referent=$row['contact_email'];
		...
		//PLAN EVAL
		$requete5='select * from egw_epce_plan_eval where id_presta = '.$id_presta.'';
		
		$resultat5 = mysql_query($requete5) or die(mysql_error());
		while($row = mysql_fetch_array($resultat5))
		{
			$this->proj=$row['descrip_proj'];
...
header('Content-disposition: inline, filename =BILAN_'.str_replace(' ','_',$nom_beneficiaire).'_'.str_replace(' ','_',$prenom_beneficiaire).'_'.$this->identifiant_pole.'.doc');
}


//Ouvre le fichier mod�le
...
//Stocke le mod�le dans une variable
$output=fread($fp, filesize($filename));

fclose($fp);

//Remplace les marqueurs du mod�le par nos donn�es

//croix
$output = str_replace('<<SOLUTION_ALT>>',$solution_alt, $output);
$output = str_replace('<<P_OUI>>',$projet_oui, $output);
$output = str_replace('<<P_NON>>',$projet_non, $output);

$output = str_replace('<<croix_negatif>>',$croix_negatif, $output);
$output = str_replace('<<croix_positif_sous_reserve>>',$croix_positif_reserve, $output);
$output = str_replace('<<croix_positif>>',$croix_positif, $output);

$output = str_replace('<<croix_an>>',$croix_an, $output);
$output = str_replace('<<croix_six>>',$croix_six, $output);
$output = str_replace('<<croix_trois>>',$croix_trois, $output);
//*****adressbook*****
...
$output = str_replace('<<SIRET>>','44082666700030', $output);
...
if($this->tmp_fin == 0)
$signature=date("dmy",time());
elseif(time()>$this->tmp_fin)
{
...
$output = str_replace('<<NUMMARCHE>>',$this->n_marche, $output);
$output = str_replace('<<NUMLOT>>',$this->n_lot, $output);

//
$output = str_replace('<<E4_DATE>>',$signature, $output);
$output = str_replace('<<E4_DATE_BIS>>',$signature, $output);
//PROJET
...
$e4bis=date('dmy',$this->cal_4);
}

if($this->cal_5==0)
{$d_5=NULL;$e5=NULL;}
else
{$d_5=date('H:i',$this->cal_5);
$e5=date('w/d/n/Y',$this->cal_5);
$e5=$this->date_fr_j($e5);
$e5bis=date('dmy',$this->cal_5);
}

if($this->cal_6==0)
{$d_6=NULL;$e6=NULL;}
else
{$d_6=date('H:i',$this->cal_6);
$e6=date('w/d/n/Y',$this->cal_6);
$e6=$this->date_fr_j($e6);
$e6bis=date('dmy',$this->cal_6);
}
...
$output = str_replace('<<HH>>',$d1, $output);
$output = str_replace('<<PP>>',$d2, $output);
$output = str_replace('<<ZZ>>',$d3, $output);
$output = str_replace('<<VV>>',$d4, $output);

//PROGRAMME D'EVALUATION
$output = str_replace('<<E1_DATE>>',$e2bis, $output);
//$output = str_replace('<<COMMENT_REF>>',utf8_decode($pt_date2), $output);
$output = str_replace('<<E2_DATE>>',$e3bis, $output);
//$output = str_replace('<<COMMENT_REF>>',utf8_decode($diagnostic1_date2), $output);
$output = str_replace('<<E3_DATE>>',$e4bis, $output);
//$output = str_replace('<<COMMENT_REF>>',utf8_decode($diagnostic2_date2), $output);
$output = str_replace('<<E5_DATE>>',$e5bis, $output);

$output = str_replace('<<ENT1>>',$e1bis, $output);
$output = str_replace('<<ENT2>>',$e2bis, $output);
$output = str_replace('<<ENT3>>',$e3bis, $output);
$output = str_replace('<<ENT4>>',$e4bis, $output);
$output = str_replace('<<ENT5>>',$e5bis, $output);
$output = str_replace('<<ENT6>>',$e6bis, $output);
...
$output = str_replace('<<DIAGNOSTIC0>>',$diagnostic0, $output);
...
$sujet2='BILAN_'.$nom_beneficiaire.'_'.$prenom_beneficiaire.' envoye au GED';
$sujet3='BILAN_'.$nom_beneficiaire.'_'.$prenom_beneficiaire.'(sauvegarde)';
if($this->email_siege!=NULL)
{
	//$this->form($sujet3,$sujet,$output,$account_email);
//$this->mail2($account_email,"presta@apsie.org",$sujet2,$sujet,$output,'','',$account_email);
//$this->mail2($account_email,"drenault@apsie.org",$sujet2,$sujet,$output,'','',$account_email);

//'Le bilan a �t� transmis a bilans-prestation.061@poleemploi.extelia.fr'
//$this->mail2($account_email,"presta@apsie.org",$sujet3,$sujet,$output,'','',$account_email);
//$this->mail2($account_email,"drenault@apsie.org",$sujet3,$sujet,$output,'','',$account_email);
...
$output = str_replace('<<DATEFIN>>',$fin_epce, $output);
$output = str_replace('<<NUMMARCHE>>',$this->n_marche, $output);
$output = str_replace('<<n_lot>>',$this->n_lot, $output);
...
//
$output = str_replace('<<DATE>>',date('dmy'), $output);
$output = str_replace('<<DATE_BILAN>>',date('dmy',$this->cal_6), $output);
...
/*//OBJECTIF
$output = str_replace('<<O1>>','Entr�e , Coherence Homme projet', $output);
$output = str_replace('<<O2>>','Diagnostic et plan d actions commercial et financier', $output);
$output = str_replace('<<O3>>','Diagnostic et plan d actions juridique et r�glementaire', $output);
...
$this->mail2("qualite@apsie.org",$this->email_bureau_prescripteur,$sujet,$sujet.'.doc',$output,"","",$account_email);
//$this->mail2("qualite@apsie.org",$this->email_bureau_prescripteur,$sujet,$sujet.'.doc',$output,"","",$account_email);
...
//Remplace les marqueurs du mod�le par nos donn�es
$n_march=$this->n_marche;
$presta='APSIE';
$siret_prest=$this->siret_form;
$siret_titu=$this->siret_form;
$n_lot=$this->n_lot;
$titulaire='APSIE';
if($this->date_fin_epce=="01/01/1970")
{$fin_epce=NULL;}
...
$output = str_replace('<<CORRES_ANPE>>',utf8_decode($this->nom_p.' '.$this->prenom_p), $output);
$output = str_replace('<<NAME1>>',utf8_decode($nom_beneficiaire), $output);
$output = str_replace('<<NAME2>>',utf8_decode($prenom_beneficiaire), $output);
...
$voir = str_replace('<<ACTION>>','Cr�ation d\'entreprise', $voir);
...
function imprimer_totalite($id_beneficiaire,$id_referent)
	...
	$requete2='SELECT * FROM  egw_epce_aspect_commercial where id_presta='.$this->id_presta.'';
		$resultat2 = mysql_query($requete2) or die(mysql_error());
		while($row = mysql_fetch_array($resultat2))
		{
	...
	$requete3='SELECT * FROM  egw_epce_forme_juridique where id_presta='.$this->id_presta.'';
	...
	$requete4='select * from egw_epce_coherence_hp where id_presta='.$this->id_presta.'';
		...
		while($row = mysql_fetch_array($resultat4))
		{
			...
			$diagnostic0=$row['diagnostic'];
			
		}
		
		//-------------
		$requete5='select * from egw_epce_aspect_financier where id_presta='.$this->id_presta.'';
		$resultat5 = mysql_query($requete5) or die(mysql_error());
		while($row = mysql_fetch_array($resultat5))
		{
	...
	/*$requete='SELECT * FROM  egw_contact_extra  where contact_name="Numero lettre de commande" and contact_id='.$id_beneficiaire.'';
		...
		$requete5='select * from egw_epce_plan_eval where id_presta='.$this->id_presta.'';
		$resultat5 = mysql_query($requete5) or die(mysql_error());
		while($row = mysql_fetch_array($resultat5))
		{
		...
		$requete='SELECT * FROM  egw_epce_bilan_avis  where id_presta='.$this->id_presta.'';
		$resultat = mysql_query($requete) or die(mysql_error());
		while($row = mysql_fetch_array($resultat))
		{
		...
		$requete='SELECT * FROM  egw_projet  where id_ben='.$id_beneficiaire.' order by id_projet desc limit 1';
		$resultat = mysql_query($requete) or die(mysql_error());
		while($row = mysql_fetch_array($resultat))
		{
	...
	function imprimer_totalite_emargement($id_beneficiaire,$id_referent,$id_presta)
	...
	$requete='SELECT d.*,pr.* FROM  egw_prestation p 
	left join egw_dispositif d on d.id_dispositif = p.id_dispositif
	left join egw_prestataire pr on pr.id_prestataire = p.id_prestataire  where id_presta='.$id_presta.'';
	//die($requete);
		$resultat = mysql_query($requete) or die(mysql_error());
		while($row = mysql_fetch_array($resultat))
		{
			$mar_n=$row['numero_marche'];
			$presta=$row['nom_dispositif'];
			$nom_presta=$row['nom'];
 ...
 $mail->headers = 'Return-Path: ' . $ad_exp ;
...
// Chaine permettant de diff�rencier les diff�rentes parties du mail
$limite = '_parties_'.md5(uniqid (rand()));
$headers.= " boundary=\"----=$limite\"\n\n";

// Premi�re partie, corps du mail en HTML
$texte = "------=$limite\n";
...
mail($destination, $nom, $texte, $headers.$attachement);
	...
	// function form($sujet,$sujet_bilan,$output_bilan,$des)
		function form()
	{


include("/data/html/egw_apsie_143/Classes/PHPExcel/IOFactory.php");




if (!file_exists("../doc/form_bilanprest_.xls")) {
...
$objPHPExcel = PHPExcel_IOFactory::load("../doc/form_bilanprest_.xls");
...
->setFormatCode('00000000000000'); */
$objPHPExcel->getActiveSheet()->getStyle('C11')->getNumberFormat()
->setFormatCode('@');

//$sheetupa->setCellValue('C8',$this->siret_form);
//$sheetupa->setCellValue('C9',"qualite@apsie.org");
...
//$sheetupa->setCellValue('C13',$this->siret_form);//35284977100039
$sheetupa->setCellValue('C14',$this->lalettre);
$sheetupa->setCellValue('C15',$this->date_debut_epce_form);
...
/*$objWriter->save('../doc/form_bilanprest.xls');
$fp=fopen('../doc/form_bilanprest.xls', "r");
$output=fread($fp, filesize('../doc/form_bilanprest.xls'));
fclose($fp);*/
/*$fp_=fopen('../doc/BILAN_PRADIER_Valerie_3204372N.doc', "r");
$output_=fread($fp_, filesize('../doc/BILAN_PRADIER_Valerie_3204372N.doc'));
fclose($fp_);*/

/*
$this->mail2("qualite@apsie.org","timsita@free.fr","BILAN_PRADIER_Valerie (test)","form_bilanprest.xls",$output,"","","qualite@apsie.org");
$this->mail2("qualite@apsie.org","drenault@apsie.org","BILAN_PRADIER_Valerie (test)","form_bilanprest.xls",$output,"","","qualite@apsie.org");
$this->mail2("qualite@apsie.org","bilans-prestation.061@poleemploi.extelia.fr","BILAN_PRADIER_Valerie (test)","form_bilanprest.xls",$output,"","","qualite@apsie.org");*/


//$this->mail2("qualite@apsie.org","bilans-prestation.061@poleemploi.extelia.fr",$sujet,$sujet_bilan,$output_bilan,"form_bilanprest.xls",$output,"qualite@apsie.org");
/*$this->mail2($des,"presta@apsie.org",$sujet,$sujet_bilan,$output_bilan,"form_bilanprest.xls",$output,$des);
$this->mail2($des,"drenault@apsie.org",$sujet,$sujet_bilan,$output_bilan,"form_bilanprest.xls",$output,$des);*/
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=680 style="border-collapse:
 ...
 <tr height=21 style="height:15.75pt">
  ...
  <td class=xl2530483 colspan=2>P�le emploi : Gestion des bilans de prestation</td>
 </tr>
 <tr height=17 style="height:12.75pt">
  <td height=17 class=xl1530483 style="height:12.75pt"></td>
  <td class=xl1530483></td>
 ...
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  ...
  <td class=xl2930483>44082666700030</td>
 </tr>
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  <td height=33 class=xl1530483 style="height:24.75pt"></td>
 ...
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  <td height=33 class=xl1530483 style="height:24.75pt"></td>
  <td class=xl2830483>N� de d�partement de r�sidence du DE</td>
  <td class=xl3130483>'.substr($this->code_postal_ben, 0, -3).'</td>
 </tr>
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  <td height=33 class=xl1530483 style="height:24.75pt"></td>
  <td class=xl3030483>Siret prestataire</td>
  <td class=xl3230483>44082666700030</td>
 </tr>
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  <td height=33 class=xl1530483 style="height:24.75pt"></td>
 ...
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  <td height=33 class=xl1530483 style="height:24.75pt"></td>
  <td class=xl3030483>Date de d�marrage</td>
 ...
 <tr height=33 style="mso-height-source:userset;height:24.75pt">
  <td height=33 class=xl1530483 style="height:24.75pt"></td>
  <td class=xl2830483>Date �ventuelle d"abandon</td>
...
$worksheet->set_column('A:B', 25); // // le 30 repr�sente la largeur de chaque colonne  
...
$worksheet->write("C8","44082666700030",$heading_l12);
...
$worksheet->write("C15",''.$this->date_debut_epce_form.'',$heading_l2);
